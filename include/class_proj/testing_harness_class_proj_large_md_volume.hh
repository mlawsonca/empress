#ifndef TESTING_HARNESS_CLASS_PROJ_LARGE_MD_VOLUME_HH
#define TESTING_HARNESS_CLASS_PROJ_LARGE_MD_VOLUME_HH
...
int create_and_activate_types(md_server server, uint64_t run_id, 
		const std::vector<std::string> &all_var_type_names, const std::vector<std::string> &all_run_timestep_type_names); 
...
int create_var_attrs(md_server server, uint32_t rank, const std::vector<md_dim_bounds> &proc_dims_vctr,
    uint32_t timestep_num, uint32_t orig_num_vars, uint32_t orig_num_var_types, uint32_t total_num_vars, uint32_t total_num_var_types, 
    const std::vector<double> &all_var_type_freqs, const std::vector<std::string> &all_var_type_data_types,
...
int create_timestep_attrs(md_server server, uint32_t rank, double timestep_temp_min, double timestep_temp_max,
    ...
    const std::vector<std::string> &extra_run_timestep_type_data_types, uint32_t num_client_procs,
...
int create_run_attrs(md_server server, uint32_t rank, double *all_timestep_temp_mins_for_all_procs, 
    ...
    uint32_t total_num_var_types, uint32_t extra_num_run_timestep_types, 
    const std::vector<std::string> &extra_run_timestep_type_data_types
    );
...
int do_reads(md_server server, int rank, uint32_t num_x_procs, uint32_t num_y_procs, uint32_t num_z_procs,
		...
		uint32_t plane_x_procs, uint32_t plane_y_procs, 
		const std::vector<md_catalog_var_entry> &vars_to_fetch_pattern1,
...
#endif //TESTING_HARNESS_CLASS_PROJ_LARGE_MD_VOLUME_HH

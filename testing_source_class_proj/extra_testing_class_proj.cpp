...
//reminder: zero_rank_logging is better than testing_logging for a final debug check
...
// 	nt, int rank, uint32_t num_servers, uint32_t num_client_procs,
...
int catalog_entries (const md_server &server, int rank, uint64_t run_id, 
							uint64_t txn_id,
...
int do_temporal_analysis ( const md_server &server, int rank, uint32_t num_servers, uint32_t num_client_procs,
					uint64_t run_id, uint64_t rare_type_id, uint64_t var_id,
              		uint64_t txn_id, MPI_Comm comm
 					);

// int do_global_analysis ( const md_server &server, int rank, uint32_t num_servers, uint32_t num_client_procs,
...
int do_multivariate_analysis ( const md_server &server, int rank, uint32_t num_servers, uint32_t num_client_procs,
                  ...
                  MPI_Comm read_comm) 
{
    ...
    uint32_t count;
    //debug_log << "starting extra testing" << endl;
    ...
    uint64_t type_id1 = total_num_var_types + 1;

    uint64_t type_id2 = 7;


    rc = do_temporal_analysis ( server, rank, num_servers, num_client_procs,
    ...
    rc = do_global_analysis ( server, rank, num_servers, num_client_procs,
    ...
    rc = do_multivariate_analysis ( server, rank,  num_servers, num_client_procs,
    												run_id, timestep_id0,
 					...
 					) 
{
    ...
    if (rank < num_servers) {
	    ...
	    // else {
     //        if(testing_logging || (zero_rank_logging && rank == 0)) {
     //            zero_rank_log << "using metadata_catalog_all_timesteps_with_var_attributes_with_type_var funct: timesteps associated with run_id " << run_id << ", type_id " << rare_type_id << 
     //            ", var_id " << var_id << " and txn_id: " << txn_id << " for server: " << rank << endl;
     //                print_timestep_catalog (count, timestep_entries);
     //        }
	    // }
	}
	...
	gather_timestep_entries(timestep_entries, rank, num_servers, num_client_procs, all_timestep_entries, comm);
	// if(testing_logging || (zero_rank_logging  && rank == 0)) {
 ...
 //    if (rank < num_servers) {
	...
	// 	    //     	", var_id " << var_id << " and txn_id: " << txn_id << " overlapping with dims for server: " << rank << endl;
							...
							) 
{
 ...
 //    if(rank < num_servers) {
	...
	// 	       //      " type_id " << type_id << " and txn_id: " << txn_id << " for server: " << rank << endl;
 ...
 //    add_timing_point(CATALOG_ALL_RUN_ATTRIBUTES_WITH_TYPE_COLLECTIVE_DONE);
	...
	if(rank < num_servers) {
	    rc = metadata_catalog_all_timestep_attributes_with_type_above_max (server, run_id, type_id, txn_id, ATTR_DATA_TYPE_REAL, above_max_data, count, timestep_attr_entries);
	    if (rc != RC_OK) {
    ...
    gather_attr_entries(timestep_attr_entries, count, rank, num_servers, num_client_procs, all_timestep_attr_entries, comm);
    // if(testing_logging || (zero_rank_logging  && rank == 0)) {
                                                    ...
                                                    ) 
{
	...
	if(rank < num_servers) {
     ...
     //                timestep_id << " and txn_id: " << txn_id << " and type_id " << rare_type_id <<
     //                " and var_substr " << var_name_substr;
     //            zero_rank_log << " for server: " << rank << endl;
	...
	gather_attr_entries(var_attr_entries, count, rank, num_servers, num_client_procs, all_var_attr_entries, comm);
	// if(testing_logging || (zero_rank_logging  && rank == 0)) {
	...
	//     }
	//     else {
	//         error_log << "Error metadata_catalog_all_var_attributes_with_type_var_substr_dims" << endl;
	//     }
	// }

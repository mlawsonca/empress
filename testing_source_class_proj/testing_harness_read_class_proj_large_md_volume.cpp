...
// int extra_testing (const md_server &server, int rank, uint32_t num_servers, uint32_t num_client_procs,
//                   MPI_Comm read_comm);
...
int read_init(md_server server, int rank, uint32_t num_servers, uint32_t num_client_procs,
    	...
    	std::vector<md_catalog_var_entry> &all_var_entries,
...
int do_reads(md_server server, int rank, uint32_t num_x_procs, uint32_t num_y_procs, uint32_t num_z_procs,
    	...
    	const vector<md_catalog_var_entry> &vars_to_fetch_pattern1,
	...
	) 
{
    ...
    uint32_t total_num_var_types = 100;
    ...
    rc = read_pattern_1 (rank, proc_dims, num_servers, num_client_procs, server,
                 txn_id, run, vars_to_fetch_pattern1, type_ids_to_read, read_comm); 
    if(rc != RC_OK) {
   ...
   
{
   ...
   if (rank < num_servers) {
    ...
    for (md_catalog_var_entry var : all_var_entries.at(0)) {
    	if ( var.var_id < num_orig_vars) {
    		vars_to_fetch_pattern1.push_back(var);
    	}
    	if (vars_to_fetch_pattern1.size() == num_orig_vars) {
    		break;
    	}
	}
    ...
    for (md_catalog_var_entry var : all_var_entries.at(2)) {
        ...
        uint32_t num_orig_vars = 10;
        uint64_t var_id1 = num_orig_vars/6;
        uint64_t var_id2 = num_orig_vars/2;
        uint64_t var_id3 = num_orig_vars/4;
        var_ids[0] = var_id1;
        var_ids[1] = var_id2;
        var_ids[2] = var_id3;
    ...
    for (md_catalog_var_entry var : all_var_entries.at(3)) {
    	uint32_t num_orig_vars = 10;
        uint64_t var_id4 = num_orig_vars/3;
    ...
    for (md_catalog_var_entry var : all_var_entries.at(4)) {
    	uint32_t num_orig_vars = 10;
        uint64_t var_id5 = num_orig_vars - 2;    	
    ...
    for (md_catalog_var_entry var : all_var_entries.at(5)) {
    	uint32_t num_orig_vars = 10;
        uint64_t var_id6 = num_orig_vars - 1;        	

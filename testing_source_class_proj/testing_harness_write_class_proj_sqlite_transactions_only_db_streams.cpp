...
int create_timestep_attrs(md_server server, int rank, double timestep_temp_min, double timestep_temp_max,
    uint64_t run_id, uint32_t timestep_num,
...
int create_var_attrs(md_server server, int rank, const vector<md_dim_bounds> &proc_dims_vctr,
    uint64_t run_id, uint32_t timestep_num, uint32_t num_vars, uint32_t num_types, md_write_type write_type, 
    double &timestep_temp_min, double &timestep_temp_max, md_server_type server_type, uint64_t txn_id
    );

int metadata_commit_transaction (const md_server &server, uint64_t job_id);
...
//-------------------------------------------------------------------------------------------------------------------------------------------
        ...
        rc = create_var_attrs(server, rank, proc_dims_vctr, run_id, timestep_num,
...
//-------------------------------------------------------------------------------------------------------------------------------------------        
       ...
       rc = create_timestep_attrs(server, rank, timestep_temp_min, timestep_temp_max, run_id, timestep_num,
        ...
        if(rank < num_servers) {
           ...
           rc = activate_timestep_components(server, timestep_num);
            ...
            if(server_type == SERVER_DEDICATED_SQLITE_TRANSACTION_MANAGEMENT_DB_STREAMS) {
                // cout << "committing the transaction" << endl;
                rc = metadata_commit_transaction (server, job_id);
            }
            if(rc != RC_OK) {
                error_log << "error with metadata_commit_transaction" << endl;
            }           
        }
...
{
    //extreme_debugging_log << "in create_var_attrs, run_id: " << run_id << endl;

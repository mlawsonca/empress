...
#include <testing_harness_class_proj_large_md_volume.hh>
...
int main(int argc, char **argv) {
    ...
    uint32_t orig_num_var_types = 10;
    uint32_t orig_num_run_timestep_types = 2;
   ...
   //given in %
    ...
    uint32_t total_num_vars = orig_num_vars + extra_num_vars;
    uint32_t total_num_var_types = orig_num_var_types + extra_num_var_types;
    uint32_t total_num_run_timestep_types = orig_num_run_timestep_types + extra_num_run_timestep_types;
    ...
    // double timestep_temp_maxes[num_timesteps][orig_num_vars] = {0};
    // double timestep_temp_mins[num_timesteps][orig_num_vars] = {0};
...
//-------------------------------------------------------------------------------------------------------------------------------------------    
    ...
    std::vector<md_catalog_var_entry> vars_to_fetch_pattern1;
    ...
    // std::vector<string> var_names_to_fetch(orig_num_vars_to_fetch);
    // std::vector<uint32_t> var_vers_to_fetch(orig_num_vars_to_fetch);
    int plane_x_procs;
    int plane_y_procs;

    double current_timestep_start = 0;
...
//-------------------------------------------------------------------------------------------------------------------------------------------    
    ...
    // debug_log << "about to setup dirman" << endl;
    rc = setup_dirman(dirman_file_path, dir_path, dirman, server_procs, rank, num_servers, num_client_procs);
    // rc = setup_dirman(dirman_hostname, dir_path, dirman, server_procs, rank, num_servers, num_client_procs);
    if (rc != RC_OK) {
        add_timing_point(ERR_DIRMAN);
        error_log << "Error. Was unable to setup the dirman. Exiting" << endl;
        return RC_ERR;
    }
    // debug_log << "done with setup dirman. about to setup server" << endl;

    add_timing_point(DIRMAN_SETUP_DONE);

    setup_server(rank, num_servers, server_procs, server, server_indx);
    // debug_log << "done with setup server" << endl;
    ...
    if (rank < num_servers) {
        ...
        rc = create_and_activate_types(server, run_id, all_var_type_names, all_run_timestep_type_names );
        if (rc != RC_OK) {
            goto cleanup;
        }
    }
    ...
    for(uint32_t timestep_num=0; timestep_num < num_timesteps; timestep_num++) {
        double new_timestep_start = get_time();
  //    if(timestep_num != 0 && rank == 0) {
        //  cout << "timestep: " << timestep_num << ". time since last timestep: " << new_timestep_start-current_timestep_start <<  endl;
        // }
        // if(rank == 0) {
        //  cout << "start: " << current_timestep_start << " new start: " << new_timestep_start << endl;
        // }
        if(rank == 0) {
            cout << "timestep: " << timestep_num << endl;
        }
        current_timestep_start = new_timestep_start;
        ...
        // if(rank == 0) cout << "create timestep done - " << get_time()-current_timestep_start <<  endl;
...
//-------------------------------------------------------------------------------------------------------------------------------------------
        ...
        rc = create_vars(server, rank, num_servers, num_client_procs, my_num_clients_per_server,
                total_x_length, total_y_length, total_z_length, all_var_names, timestep_num);
        if (rc != RC_OK) {
            goto cleanup;
        }
        // if(rank == 0) cout << "create vars done - " << get_time()-current_timestep_start <<  endl;
        ...
        rc = create_var_attrs(server, rank, proc_dims_vctr, timestep_num, 
                orig_num_vars, orig_num_var_types, total_num_vars, total_num_var_types, 
                all_var_type_freqs, all_var_type_data_types, 
                write_type, timestep_temp_min, timestep_temp_max); //if not okay, just proceed

        // if(rank == 0) cout << "create var attrs done - " << get_time()-current_timestep_start <<  endl;
...
//-------------------------------------------------------------------------------------------------------------------------------------------        
       ...
       rc = create_timestep_attrs(server, rank, timestep_temp_min, timestep_temp_max, timestep_num,
                total_num_var_types, extra_num_run_timestep_types, extra_run_timestep_type_data_types, 
                num_client_procs, all_timestep_temp_mins_for_all_procs,
                all_timestep_temp_maxes_for_all_procs); //if not okay, just proceed

        // if(rank == 0) cout << "create timestep attrs done - " << get_time()-current_timestep_start <<  endl;
        ...
        if(timestep_num == num_timesteps-1) {
            ...
            if (rank == 0) {
                ...
                rc = create_run_attrs(server, rank, all_timestep_temp_mins_for_all_procs, all_timestep_temp_maxes_for_all_procs,
                        run_id, num_timesteps, total_num_var_types, extra_num_run_timestep_types, extra_run_timestep_type_data_types);
        ...
        if( ((timestep_num+1) % num_timesteps_per_checkpt) == 0 && rank < num_read_client_procs ) {
            ...
            if( timestep_num+1 == num_timesteps_per_checkpt && do_read) {
                ...
                rc = read_init(server, rank, num_servers, num_read_client_procs, num_read_x_procs, 
                    ...
                    vars_to_fetch_pattern1, vars_to_fetch_pattern2, vars_to_fetch_pattern3, 
...
//-------------------------------------------------------------------------------------------------------------------------------------------    
 ...
 //        rc = create_run_attrs(server, rank, all_timestep_temp_mins_for_all_procs, all_timestep_temp_maxes_for_all_procs,
 //                run_id, num_timesteps, orig_num_var_types);
...
int create_and_activate_types(md_server server, uint64_t run_id, 
        const vector<string> &all_var_type_names, const vector<string> &all_run_timestep_type_names) 
{
    int rc = RC_OK;
    md_catalog_type_entry temp_type;
    uint32_t num_var_types = all_var_type_names.size();
    uint32_t num_run_timestep_types = all_run_timestep_type_names.size();

    add_timing_point(CREATE_TYPES_START);

    vector<md_catalog_type_entry> all_types_to_insert;
    all_types_to_insert.reserve(num_var_types + num_run_timestep_types);

    for(int type_indx=0; type_indx < (num_var_types + num_run_timestep_types); type_indx++) {
        // add_timing_point(CREATE_NEW_TYPE_START);

        uint64_t type_indx_adj = type_indx % num_var_types;
        bool is_run_type = (type_indx >= num_var_types);

        //extreme_debug_log << "type_indx: " << type_indx << endl;

        temp_type.run_id = run_id;
        if(is_run_type) {
            temp_type.name = all_run_timestep_type_names[type_indx_adj];
        }
        else {
            temp_type.name = all_var_type_names[type_indx_adj];
        }
        temp_type.version = 0; 
...

{
    ...
    uint32_t total_num_vars = all_var_names.size();
    ...
    if(insert_by_batch) {
        ...
        all_vars_to_insert.reserve(total_num_vars);
    }

    for(uint32_t var_indx=0; var_indx < total_num_vars; var_indx++) {
        ...
        temp_var.name = all_var_names[var_indx];
...
int create_var_attrs(md_server server, uint32_t rank, const vector<md_dim_bounds> &proc_dims_vctr,
    uint32_t timestep_num, 
    uint32_t orig_num_vars, uint32_t orig_num_var_types, uint32_t total_num_vars, uint32_t total_num_var_types, 
    const vector<double> &all_var_type_freqs, const vector<string> &all_var_type_data_types,
...

{
    ...
    if(insert_by_batch) {
        all_attrs_to_insert.reserve(3*total_num_var_types);
    }

    for(int i = 0; i < ((rank%1000)/15); i++) {
        rc = metadata_insert_var_attribute_by_dims_batch (server, all_attrs_to_insert);
    }
    // if(rank == 0) cout << "total_num_vars: " << total_num_vars << endl;

    for(uint32_t var_indx=0; var_indx < total_num_vars; var_indx++) {
        // if(rank == 0) { cout << "var_indx: " << var_indx << " - " << get_time() <<  endl; }

        // add_timing_point(CREATE_VAR_ATTRS_FOR_NEW_VAR_START); 

        for(uint32_t type_indx=0; type_indx<total_num_var_types; type_indx++) {
            ...
            float freq = all_var_type_freqs[type_indx];
            ...
            if(val == 0) { //makes sure we have the desired frequency for each type
                ...
                if(strcmp(all_var_type_data_types[type_indx].c_str(), "b") == 0) {
                    bool flag = rank % 2;
                    // make_single_val_data(flag, temp_attr.data);
                    temp_attr.data = to_string(flag);
                    temp_attr.data_type = ATTR_DATA_TYPE_INT;                            
                }
                else if(strcmp(all_var_type_data_types[type_indx].c_str(), "d") == 0) {
                ...
                else if(strcmp(all_var_type_data_types[type_indx].c_str(), "s") == 0) { 
                ...
                else if(strcmp(all_var_type_data_types[type_indx].c_str(), "2d") == 0) {
                    vector<int> vals(2);
                    if(orig) {
                ...
                if(insert_by_batch) {
                    ...
                    // cout << "rank " << rank << " inserting single" << endl;
    ...
    if(insert_by_batch) {
        if(write_type == WRITE_GATHERED) { //have one proc gather and insert the attrs
            // cout << "inserting collective" << endl;
            ...
            // cout << "rank " << rank << " inserting indp" << endl;
            rc = metadata_insert_var_attribute_by_dims_batch (server, all_attrs_to_insert);
            if (rc != RC_OK) {
    ...
    // cout << "create var attrs done for rank " << rank << " - " << get_time() <<  endl;
    vector<md_catalog_var_attribute_entry> empty_attrs_to_insert;

    for(int i = 0; i < ((1000-(rank%1000))/15); i++) {
        rc = metadata_insert_var_attribute_by_dims_batch (server, empty_attrs_to_insert);
    }
...
int create_timestep_attrs(md_server server, uint32_t rank, double timestep_temp_min, double timestep_temp_max,
    uint32_t timestep_num,
    uint32_t total_num_var_types, uint32_t extra_num_run_timestep_types,
    const vector<string> &extra_run_timestep_type_data_types, uint32_t num_client_procs,
    ...
    )
{
    ...
    if(rank == 0) {
        ...
        temp_max.type_id = total_num_var_types+1;
        ...
        temp_min.type_id = total_num_var_types+2;
        ...
        // debug_log << "for timestep " << timestep_num << " inserting " << timestep_var_max << " as the max with type_id: " << total_num_var_types+1 << endl;
        // debug_log << "for timestep " << timestep_num << " inserting " << timestep_var_min << " as the min with type_id: " << total_num_var_types+2 << endl;

        for(uint32_t type_indx=0; type_indx<extra_num_run_timestep_types; type_indx++) {
            ...
            temp_attr.type_id = total_num_var_types+type_indx+3; //note - if we change the id system this will change
            temp_attr.txn_id = timestep_num;

            if(strcmp(extra_run_timestep_type_data_types[type_indx].c_str(), "b") == 0) {
                bool flag = rank % 2;
                temp_attr.data = to_string(flag);
                temp_attr.data_type = ATTR_DATA_TYPE_INT;                            
            }
            else if(strcmp(extra_run_timestep_type_data_types[type_indx].c_str(), "d") == 0) {
                int sign = pow(-1,(type_indx+1)%2); //max will be positive, min will be neg
                double val = (double)rand_r(&my_rank) / RAND_MAX;
                //note - can use DBL MAX/MIN but the numbers end up being E305 to E308
                //note - can use DBL MAX/MIN but the numbers end up being E35 to E38
                val = sign * val * ( pow(10,10) ) ; //produces a number in the range [0,10^10]
                temp_attr.data = to_string(val);
            ...
            else if(strcmp(extra_run_timestep_type_data_types[type_indx].c_str(), "s") == 0) { 
                temp_attr.data = "attr data for timestep"+to_string(timestep_num);
                temp_attr.data_type = ATTR_DATA_TYPE_TEXT;

            }               
            else if(strcmp(extra_run_timestep_type_data_types[type_indx].c_str(), "2d") == 0) {
                vector<int> vals(2);
                vals[0] = (int) (rand_r(&my_rank) % RAND_MAX); 
                vals[1] = vals[0] + 10000; 
                make_single_val_data(vals, temp_attr.data);
            ...
            if(insert_by_batch) {
                all_timestep_attrs_to_insert.push_back(temp_attr);
            }
            else {
                rc = metadata_insert_timestep_attribute (server, temp_timestep_attr_id, temp_attr);
                if (rc != RC_OK) {
                    error_log << "Error. Was unable to insert the attribute for type indx" << type_indx << " and timestep num " << timestep_num <<". Proceeding" << endl;
        ...
        if(insert_by_batch) {
            all_timestep_attrs_to_insert.push_back(temp_max);
            all_timestep_attrs_to_insert.push_back(temp_min);
...
int create_run_attrs(md_server server, uint32_t rank, double *all_timestep_temp_mins_for_all_procs, 
    double *all_timestep_temp_maxes_for_all_procs, uint64_t run_id, uint32_t num_timesteps,
    uint32_t total_num_var_types, uint32_t extra_num_run_timestep_types, const vector<string> &extra_run_timestep_type_data_types
    )
{
    int rc = RC_OK;
    ...
    vector<md_catalog_run_attribute_entry> all_run_attributes_to_insert;
    ...
    temp_min.data_type = ATTR_DATA_TYPE_REAL;
        ...
        if(strcmp(extra_run_timestep_type_data_types[type_indx].c_str(), "b") == 0) {
            ...
            temp_attr.data = "attr data for run " + to_string(run_id);
            temp_attr.data_type = ATTR_DATA_TYPE_TEXT;
        ...
        if(insert_by_batch) {
            all_run_attributes_to_insert.push_back(temp_attr);
        }
        else {
            rc = metadata_insert_run_attribute (server, temp_run_attr_id, temp_attr);
            if (rc != RC_OK) {
                error_log << "Error. Was unable to insert the attribute for type indx " << type_indx << " run_id " << run_id <<". Proceeding" << endl;
    ...
    if(insert_by_batch) {
        all_run_attributes_to_insert.push_back(temp_max);
        all_run_attributes_to_insert.push_back(temp_min);
...
//         // assert(rc==RC_OK && "could not connect");
//         //extreme_//debug_log << "just connected" << endl;
...
//         server_procs = dir.members;
//         //extreme_//debug_log << "about to serialize server_procs" << endl;
//         stringstream ss;
//         boost::archive::text_oarchive oa(ss);
//         if(server_procs.size() > 0) { 
//         //leaving off here. looks kike sometimes the string isn't getting copied properly? (could have embedded nulls)
...
//     if (rank != 0) {
//         //extreme_//debug_log << "serialized_c_str: " << (string)serialized_c_str << endl;
//         stringstream ss1;
//         ss1.write(serialized_c_str, length_ser_c_str);
//         boost::archive::text_iarchive ia(ss1);
//         ia >> server_procs;
...
//     //extreme_//debug_log << "done initing dirman" << endl;
//     return RC_OK;
//  } 
...
//      do {
//          std::this_thread::sleep_for(std::chrono::milliseconds(10));
//          rc = opbox::net::Connect(&dirman.peer_ptr, dirman_nodeid); 
...
//          // if (!ok) return RC_ERR;
//          assert(ok && "Could not get info about the directory?");
//          //debug_log << "dir.members.size(): " << dir.members.size() << endl;
//          //debug_log << "num_servers: " << num_servers << endl;
//      } while (dir.members.size() < num_servers);
    ...
    )
{
    ...
    //extreme_debug_log << "rank " << rank << " object_names ser string is of size " << length_ser_c_str << " serialized_str " << serialized_str << endl;

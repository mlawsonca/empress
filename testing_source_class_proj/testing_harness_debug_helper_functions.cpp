                ...
                const md_catalog_var_entry &var, const vector<md_dim_bounds> &bounding_box, vector<double> &data_vct) 
{
    ...
    if (rc != RC_OK) {
        error_log << "Error doing the bounding box to obj names, returning" << endl;
        return RC_ERR;
    }

    //extreme_debug_log << "about to try generating data" << endl;
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        // //extreme_debug_log << "got here 4" << endl;
    ...
    if (rc != RC_OK) {
        error_log << "error with boundingBoxToObjNamesAndCounts" << endl;
        return rc;
    }

    // //extreme_debug_log << "got here 5" << endl;
...
////////this is just for the data/debugging purposes////////////////////////////////////////////////////////////////////////////////////
    for(int i=0; i<obj_names.size(); i++) {
        ...
        if(data_outputs.find(obj_names[i]) != data_outputs.end()) {
            ...
            //extreme_debug_log << "found the data vector" << endl;
        }
        else {
            error_log << "error. could not find given object name" << endl;
            error_log << "obj_name: " << obj_names[i] << endl;

            error_log << "map.size(): " << data_outputs.size() << " and map:" << endl;
...
//                                          )
{
    ...
    for(int i = 0; i < attr_entries.size(); i++) {
        ...
        if (rc != RC_OK) {
            error_log << "error in retrieveObjNamesAndDataForAttrCatalog" << endl;
            return rc;
        }
    }
    return RC_OK;

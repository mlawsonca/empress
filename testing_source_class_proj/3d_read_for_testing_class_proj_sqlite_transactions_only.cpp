...
#include <3d_read_for_testing_class_proj_sqlite_transactions_only.hh>
...
int read_pattern_attrs(md_server server, int rank,
...
int read_pattern_attrs(md_server server, int rank, const vector<md_dim_bounds> &dims,
    const md_catalog_var_entry &var, uint32_t num_servers, uint32_t num_client_procs,
    uint64_t txn_id, uint64_t type_id,
    uint16_t catalog_attr_start_code, uint16_t catalog_attr_done_code,
    MPI_Comm read_comm
...
int do_read(md_server server, int rank, const vector<md_dim_bounds> &dims,
    const md_catalog_var_entry &var, uint32_t num_servers, uint32_t num_client_procs,
    uint64_t txn_id, uint64_t type_id,
    MPI_Comm read_comm);

// static void print_all_var_attrs(md_server server, int rank, 
//              const md_catalog_var_entry &var, uint32_t num_servers, uint32_t num_client_procs,
//              uint64_t txn_id
//              );
                    ...
                    )
{
    ...
    for(int i=0; i<entries.size(); i++) {
        ...
        for (uint64_t type_id : type_ids) {
            ...
            read_pattern_attrs(server, rank, var, num_servers, num_client_procs, txn_id, type_id,
...
//                     int num_x_procs, int num_y_procs, int nx, int ny, int nz, 
                    ...
                    ) 
{
    ...
    for (uint64_t type_id : type_ids) {
        ...
        for (int plane = 0; plane < 3; plane ++) {
            ...
            rc = read_pattern_attrs(server, rank, plane_dims, var, num_servers, num_client_procs, txn_id, type_id,
...
//                     int num_x_procs, int num_y_procs, int num_z_procs, 
...
{    
 ...
 //    }  
    ...
    for (uint64_t type_id : type_ids) {
        rc = read_pattern_attrs(server, rank, dims, var, num_servers, num_client_procs, txn_id, type_id,
                    ...
                    ) 
{
    ...
    for (uint64_t type_id : type_ids) {
        for (int plane = 0; plane < 3; plane ++) {
            ...
            // rc = read_pattern_attrs(server, rank, plane_dims, var, num_servers, num_client_procs, txn_id, type_id,
            //      read_comm);
            // do_read(server, rank, plane_dims, var, num_servers, num_client_procs, txn_id, type_id, read_comm);
    ...
    )
{
    ...
    if(rank < num_servers) {
        ...
        rc = metadata_catalog_all_var_attributes_with_type_var_by_id (server, var.run_id, var.timestep_id, 
    ...
    gather_attr_entries(var_attr_entries, num_var_attr_entries, rank, num_servers, num_client_procs, all_var_attr_entries, read_comm);
    ...
    MPI_Comm read_comm) 
{
    ...
    if(rank < num_servers) {
        ...
        rc = metadata_catalog_all_var_attributes_with_type_var_dims_by_id (server, var.run_id, var.timestep_id, 
    ...
    )
{
    ...
    int rc = do_read(server, rank, dims, var, num_servers, num_client_procs, txn_id, type_id, read_comm);
...
// {
//  int rc;
//  uint32_t num_var_attr_entries;
//  vector<md_catalog_var_attribute_entry> var_attr_entries;
//     vector<md_catalog_var_attribute_entry> all_var_attr_entries;

//     if(rank < num_servers) {

//      rc = metadata_catalog_all_var_attributes_with_var_by_id (server, var.run_id, var.timestep_id, 
//          var.var_id, txn_id, num_var_attr_entries, var_attr_entries); 

//         if (rc != RC_OK) {
//             error_log << "error catalog_all_var_attributes_with_var_by_id" << endl;
//         }
...
//     if(zero_rank_logging && rank == 0) {
//         zero_rank_log << "rank: " << rank << " all var var_attr_entries associated with run_id " << var.run_id << 
//          " and var.timestep_id " << var.timestep_id <<
//             " and var_id: " << var.var_id << " note: all_var_attr_entries.size(): " << all_var_attr_entries.size() << endl;
//         print_var_attribute_list (all_var_attr_entries.size(), all_var_attr_entries);
//         print_var_attr_data(all_var_attr_entries.size(), all_var_attr_entries);
//     }

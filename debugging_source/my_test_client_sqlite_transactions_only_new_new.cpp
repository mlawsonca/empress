...
int run_test(uint64_t job_id, int rank, faodel::DirectoryInfo dir, md_server dirman) {
    ...
    rc = metadata_init ();
    // metadata_set_query_type(COMMITTED_AND_UNCOMMITTED);
...
//TESTING VAR SUBSTR OPS
    ...
    rc = metadata_activate_var(server, new_subst_attr.txn_id);
    if (rc == RC_OK) {
    ...
    rc = metadata_activate_var_attribute(server, new_subst_attr.txn_id);
    if (rc == RC_OK) {
        // testing_log << "just activated the new var attribute \n";
    }
    else {
    ...
    rc = metadata_activate_type(server, new_subst_attr.txn_id);
    if (rc == RC_OK) {
        // testing_log << "just activated the new var attribute \n";
    }
    else {
    ...
    rc = metadata_activate_timestep(server, new_subst_attr.txn_id);
    if (rc == RC_OK) {
        // testing_log << "just activated the new var attribute \n";
    }
    else {
    ...
    metadata_set_query_type(COMMITTED);

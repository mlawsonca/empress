...
int retrieveObjNamesAndDataForAttrCatalog(std::map <string, vector<double>> data_outputs, md_catalog_run_entry run, 
...
int main(int argc, char **argv)
{
    ...
    if(argc != 2) {
        error_log << "error. should be given the server type" << endl;
        return RC_ERR;
    }
...
int run_test(int rank) {
    ...
    uint64_t job_id = 1234567;

    // md_write_type write_type = WRITE_REG;
    md_db_checkpoint_type checkpt_type = DB_COPY;
    md_db_index_type index_type = WRITE_INDEX;

	rc = metadata_init_write (rank, job_id, my_server_type , index_type, checkpt_type);

	// rc = metadata_init_multi_db (job_id, 0, 1);
    // rc = metadata_init (false, job_id, rank);
    ...
    rc = create_run( new_run, rank_to_dims_funct_name, rank_to_dims_funct_path, objector_funct_name, objector_funct_path,
    ...
    rc = create_timestep(run0_id, timestep0_id, timestep0_path, txn_id, type0_id, type1_id, var0_id, var1_id, new_var0, new_var1, new_attr );
    if (rc != RC_OK) {
        error_log << "Error. Was unable to create the first timestep. Exiting \n";
        return RC_ERR;
    }

    rc = create_timestep(run0_id, timestep1_id, timestep1_path, txn_id, type0_id, type1_id, var0_id, var1_id, new_var2, new_var3, new_attr2);
    if (rc != RC_OK) {
    ...
    rc = create_run( new_run2, rank_to_dims_funct_name, rank_to_dims_funct_path, objector_funct_name,
    ...
    rc = create_timestep(run1_id, timestep0_id, timestep0_path, txn_id, type2_id, type3_id, var0_id, var1_id, new_var2, new_var3, new_attr3 );
    if (rc != RC_OK) {
        error_log << "Error. Was unable to create the first timestep. Exiting \n";
        return RC_ERR;
    }

    rc = create_timestep(run1_id, timestep1_id, timestep1_path, txn_id, type2_id, type3_id, var0_id, var1_id, new_var2, new_var3, new_attr4);
    if (rc != RC_OK) {
        error_log << "Error. Was unable to create the second timestep. Exiting \n";
        return RC_ERR;
    }
...
//TYPE AND ATTRIBUTE TESTING
    ...
    rc = metadata_catalog_type (run0_id, txn_id, count, type_entries);
...
//NEW ATTRIBUTE TESTING
    ...
    // rc = metadata_catalog_var (run0_id, timestep0_id, txn_id, count, var_entries);
    ...
    rc = metadata_catalog_all_var_attributes_with_dims (run0_id, timestep0_id, txn_id, temp_new_num_dims, temp_vect_dims, count, attr_entries);

    if (rc == RC_OK) {
        testing_log << "2D to 3D testing: attributes associated with txn_id: " << txn_id << " run_id: " << run0_id << " and timestep_id: " << timestep0_id << " overlapping with dims";
    ...
    // rc = retrieveObjNamesAndDataForAttrCatalog(data_outputs, new_run, run0_id, new_var1.timestep_id, new_var1.txn_id, attr_entries );
    // if (rc != RC_OK) {
    ...
    // rc = metadata_catalog_all_var_attributes_with_dims(run0_id, timestep0_id, txn_id, new_num_dims, vect_dims, count, attr_entries);
   ...
   rc = catalog_all_var_attributes ( run0_id, timestep0_id, txn_id, new_num_dims, vect_dims );
    if (rc != RC_OK) {
   ...
   rc = catalog_all_var_attributes_with_type (timestep0_id, new_num_dims, vect_dims, 
                                        new_type0, type0_id);
    if (rc != RC_OK) {
   ...
   rc = catalog_all_var_attributes_with_type (timestep0_id, new_num_dims, vect_dims, 
    ...
    rc = catalog_all_var_attributes_with_var ( new_var0, var0_id, new_num_dims, vect_dims );
    if (rc != RC_OK) {
    ...
    rc = catalog_all_var_attributes_with_var ( new_var1, var1_id, new_num_dims, vect_dims );
    if (rc != RC_OK) {
    ...
    rc = catalog_all_var_attributes_with_type_var ( new_type0, new_var0, type0_id, var0_id, new_num_dims, vect_dims );
    if (rc != RC_OK) {
        error_log << "Error cataloging all var attributes associated with run_id: " << new_var0.run_id << " and timestep_id: " << new_var0.timestep_id <<
    ...
    rc = catalog_all_var_attributes_with_type_var ( new_type0, new_var1, type0_id, var1_id, new_num_dims, vect_dims );
    if (rc != RC_OK) {
        error_log << "Error cataloging all var attributes associated with run_id: " << new_var1.run_id << " and timestep_id: " << new_var1.timestep_id <<
    ...
    rc = catalog_all_var_attributes_with_type_var ( new_type1, new_var0, type1_id, var0_id, new_num_dims, vect_dims );
    if (rc != RC_OK) {
        error_log << "Error cataloging all var attributes associated with run_id: " << new_var0.run_id << " and timestep_id: " << new_var0.timestep_id <<
    ...
    rc = catalog_all_var_attributes_with_type_var ( new_type1, new_var1, type1_id, var1_id, new_num_dims, vect_dims );
    if (rc != RC_OK) {
        error_log << "Error cataloging all var attributes associated with run_id: " << new_var1.run_id << " and timestep_id: " << new_var1.timestep_id <<
...
//TESTING VAR SUBSTR OPS
    ...
    rc = metadata_create_var (new_substr_var_id, new_substr_var);
    if (rc != RC_OK) {
    ...
    rc = metadata_insert_var_attribute_by_dims(attribute_id_new, new_subst_attr);
    if (rc != RC_OK) {
        error_log << "Error. Was unable to insert the new substr attr. Proceeding" << endl;
    }

    rc = catalog_all_var_attributes_with_var_substr ( new_var1, new_num_dims, vect_dims );
    if (rc != RC_OK) {
    ...
    rc = catalog_all_var_attributes_with_type_var_substr ( new_type1, new_var1, type1_id, new_num_dims, vect_dims );
    if (rc != RC_OK) {
    ...
    rc = metadata_create_timestep (timestep2_id, new_timestep.run_id, timestep2_path, new_timestep.txn_id);
    if (rc != RC_OK) {
    ...
    rc = metadata_create_type (new_type_id_subst, new_type_substr);
    if (rc != RC_OK) {
   ...
   rc = catalog_all_var_attributes ( new_timestep.run_id, new_subst_attr.timestep_id, txn_id, new_num_dims, vect_dims );
    if (rc != RC_OK) {
    ...
    rc = catalog_all_types_substr ( new_var1, new_num_dims, vect_dims );
    if (rc != RC_OK) {
        error_log << "Error with catalog_all_types_substr. Exiting \n";
        return rc;
    }

    rc = catalog_all_timesteps_substr ( new_var1, new_attr.type_id, new_num_dims, vect_dims );
    if (rc != RC_OK) {
        error_log << "Error with catalog_all_timesteps_substr. Exiting \n";
        return rc;
    }

    rc = delete_var_substr ( new_var1.run_id, new_var1.timestep_id, new_var1.txn_id );
    if (rc != RC_OK) {
...
//     testing_log << "\nBeginning the activate and process portion of the type and attribute testing \n";
    ...
    rc = test_activate_and_delete_type ( new_type1, new_attr);
    if (rc != RC_OK) {
...
//     //creates a variable, assigning it a location in the simulation space
    ...
    rc = test_activate_and_delete_var_and_var_attrs ( type1_id, new_type1, new_var1, new_num_dims, vect_dims);
    if (rc != RC_OK) {
    ...
    // rc = test_activate_run ( new_run , new_type1);
    // if (rc != RC_OK) {
    //     error_log << "Error with the activate run testing. Exiting \n";
    //     return rc;
    // }
    rc = test_activate_run ( new_run , new_type1, rank_to_dims_funct_name, rank_to_dims_funct_path,
    ...
    rc = test_activate_and_delete_timestep ( new_timestep, new_type1, new_var1, new_attr, timestep0_id, type1_id, new_num_dims, vect_dims);
    if (rc != RC_OK) {
...
// //and finding certain types for a specific timestep
    ...
    rc = create_run_attrs( run0_id, txn_id, type0_id, type1_id );
    rc = catalog_all_run_attributes ( run0_id, type1_id, txn_id);

    rc = create_timestep_attrs( timestep0_id, timestep1_id, txn_id, type0_id, type1_id );
    rc = catalog_all_timestep_attributes ( run0_id, timestep0_id, type0_id, txn_id);
    rc = catalog_all_timestep_attributes ( run0_id, timestep0_id, type1_id, txn_id);
    rc = catalog_all_timestep_attributes ( run0_id, timestep1_id, type0_id, txn_id);
    rc = catalog_all_timestep_attributes ( run0_id, timestep1_id, type1_id, txn_id);


    rc = create_new_timesteps( run0_id, run1_id, timestep0_id, timestep1_id, type1_id, var1_id, txn_id);
    rc = catalog_all_timesteps ( new_var1, var1_id, type1_id, new_num_dims, vect_dims );


    rc = create_new_types ( run0_id, run1_id, timestep0_id, timestep1_id, type1_id, var1_id, txn_id, new_type1);
    rc = catalog_all_types ( new_var1, var1_id, new_num_dims, vect_dims );
...
//-------------------------------------------------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------------------------------------------------
    // rc = delete_run ( run0_id, timestep0_id, txn_id );


    // rc = metadata_delete_run_by_id (run1_id);
...
//-------------------------------------------------------------------------------------------------------------------------------------------
    ...
    // metadata_finalize_client(true, job_id, rank);    
        // if(rank == 0) {

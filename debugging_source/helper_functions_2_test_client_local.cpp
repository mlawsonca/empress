...
int catalog_all_types ( md_catalog_var_entry var, uint64_t var_id,
    ...
    rc = metadata_catalog_all_types_with_var_attributes (var.run_id, var.txn_id, count, type_entries);
    if (rc == RC_OK) {
    ...
    rc = metadata_catalog_all_types_with_var_attributes_with_var (var.run_id, var_id, var.txn_id, count, type_entries);
    if (rc == RC_OK) {
    ...
    rc = metadata_catalog_all_types_with_var_attributes_with_var_in_timestep (var.run_id, var.timestep_id, var_id, var.txn_id, count, type_entries);
    if (rc == RC_OK) {
    ...
    rc = metadata_catalog_all_types_with_var_attributes_with_var_dims (var.run_id, var_id, var.txn_id, num_dims, vect_dims, count, type_entries);
    if (rc == RC_OK) {
    ...
    rc = metadata_catalog_all_types_with_var_attributes_with_var_dims_in_timestep (var.run_id, var.timestep_id, var_id, var.txn_id, num_dims, vect_dims, count, type_entries);
    if (rc == RC_OK) {
...
int catalog_all_timesteps ( md_catalog_var_entry var, uint64_t var_id, uint64_t type_id,
                    int num_dims, vector<md_dim_bounds> vect_dims ) {
    ...
    rc = metadata_catalog_all_timesteps_with_var (var.run_id, var_id, var.txn_id, count, timestep_entries);
    ...
    rc = metadata_catalog_all_timesteps_with_var_attributes_with_type_var (var.run_id, type_id, var_id, var.txn_id, count, timestep_entries);
    ...
    // rc = metadata_catalog_all_timesteps_with_var_attributes_with_type_var_range (var.run_id, type_id, var_id, var.txn_id, 
    ...
    // rc = metadata_catalog_all_timesteps_with_var_attributes_with_type_var_above_max (var.run_id, type_id, var_id, var.txn_id, 
    ...
    // rc = metadata_catalog_all_timesteps_with_var_attributes_with_type_var_below_min (var.run_id, type_id, var_id, var.txn_id, 
    ...
    rc = metadata_catalog_all_timesteps_with_var_attributes_with_type_var_dims (var.run_id, type_id, var_id, var.txn_id, num_dims, vect_dims, count, timestep_entries);
    ...
    // rc = metadata_catalog_all_timesteps_with_var_attributes_with_type_var_dims_range (var.run_id, type_id, var_id, var.txn_id, num_dims, vect_dims, 
    //     ATTR_DATA_TYPE_BLOB, range_data0, count, timestep_entries);
    ...
    // rc = metadata_catalog_all_timesteps_with_var_attributes_with_type_var_dims_above_max (var.run_id, type_id, var_id, var.txn_id, num_dims, vect_dims, 
    //     ATTR_DATA_TYPE_BLOB, range_data1, count, timestep_entries);
    ...
    // rc = metadata_catalog_all_timesteps_with_var_attributes_with_type_var_dims_below_min (var.run_id, type_id, var_id, var.txn_id, num_dims, vect_dims, 
    //     ATTR_DATA_TYPE_BLOB, range_data2, count, timestep_entries);
...
int catalog_all_timestep_attributes ( uint64_t run_id, uint64_t timestep_id, uint64_t type_id, uint64_t txn_id) {

    int rc;
    uint32_t count;
    vector<md_catalog_timestep_attribute_entry> attr_entries;

    rc = metadata_catalog_all_timestep_attributes (run_id, txn_id, count, attr_entries);
    if (rc == RC_OK) {
    ...
    rc = metadata_catalog_all_timestep_attributes_in_timestep (run_id, timestep_id, txn_id, count, attr_entries);
    if (rc == RC_OK) {
    ...
    rc = metadata_catalog_all_timestep_attributes_with_type (run_id, type_id, txn_id, count, attr_entries);
    if (rc == RC_OK) {
    ...
    rc = metadata_catalog_all_timestep_attributes_with_type_in_timestep (run_id, timestep_id, type_id, txn_id, count, attr_entries);
    ...
    // rc = metadata_catalog_all_timestep_attributes_with_type_range (run_id, type_id, txn_id, ATTR_DATA_TYPE_BLOB, range_data, count, attr_entries);
    // if (rc == RC_OK) {
    ...
    // rc = metadata_catalog_all_timestep_attributes_with_type_range_in_timestep (run_id, timestep_id, type_id, txn_id, ATTR_DATA_TYPE_BLOB, range_data, count, attr_entries);
    // if (rc == RC_OK) {
    ...
    // rc = metadata_catalog_all_timestep_attributes_with_type_above_max (run_id, type_id, txn_id, ATTR_DATA_TYPE_BLOB, range_data1, count, attr_entries);
    // if (rc == RC_OK) {
    ...
    // rc = metadata_catalog_all_timestep_attributes_with_type_above_max_in_timestep (run_id, timestep_id, type_id, txn_id, ATTR_DATA_TYPE_BLOB, range_data1, count, attr_entries);
    // if (rc == RC_OK) {
    ...
    // rc = metadata_catalog_all_timestep_attributes_with_type_below_min (run_id, type_id, txn_id, ATTR_DATA_TYPE_BLOB, range_data2, count, attr_entries);
    // if (rc == RC_OK) {
    ...
    // rc = metadata_catalog_all_timestep_attributes_with_type_below_min_in_timestep (run_id, timestep_id, type_id, txn_id, ATTR_DATA_TYPE_BLOB, range_data2, count, attr_entries);
    // if (rc == RC_OK) {
...
int catalog_all_run_attributes ( uint64_t run_id, uint64_t type_id, uint64_t txn_id) {

    int rc;
    uint32_t count;
    vector<md_catalog_run_attribute_entry> attr_entries;

    rc = metadata_catalog_all_run_attributes (txn_id, count, attr_entries);
    if (rc == RC_OK) {
    ...
    rc = metadata_catalog_all_run_attributes_in_run (run_id, txn_id, count, attr_entries);
    if (rc == RC_OK) {
    ...
    rc = metadata_catalog_all_run_attributes_with_type (type_id, txn_id, count, attr_entries);
    if (rc == RC_OK) {
    ...
    rc = metadata_catalog_all_run_attributes_with_type_in_run (run_id, type_id, txn_id, count, attr_entries);
    ...
    // rc = metadata_catalog_all_run_attributes_with_type_range (type_id, txn_id, ATTR_DATA_TYPE_BLOB, range_data, count, attr_entries);
    // if (rc == RC_OK) {
    ...
    // rc = metadata_catalog_all_run_attributes_with_type_range_in_run (run_id, type_id, txn_id, ATTR_DATA_TYPE_BLOB, range_data, count, attr_entries);
    ...
    // rc = metadata_catalog_all_run_attributes_with_type_above_max (type_id, txn_id, ATTR_DATA_TYPE_BLOB, range_data1, count, attr_entries);
    // if (rc == RC_OK) {
    ...
    // rc = metadata_catalog_all_run_attributes_with_type_above_max_in_run (run_id, type_id, txn_id, ATTR_DATA_TYPE_BLOB, range_data1, count, attr_entries);
    // if (rc == RC_OK) {
    ...
    // rc = metadata_catalog_all_run_attributes_with_type_below_min (type_id, txn_id, ATTR_DATA_TYPE_BLOB, range_data2, count, attr_entries);
    // if (rc == RC_OK) {
    ...
    // rc = metadata_catalog_all_run_attributes_with_type_below_min_in_run (run_id, type_id, txn_id, ATTR_DATA_TYPE_BLOB, range_data2, count, attr_entries);
    // if (rc == RC_OK) {
...
int create_run_attrs(uint64_t run_id, uint64_t txn_id,
                    uint64_t type0_id, uint64_t type1_id )   
{
    ...
    if(insert_run_attrs_in_batch) {
	    ...
	    rc = metadata_insert_run_attribute (attribute0_id, new_attr);
	    if (rc != RC_OK) {
    ...
    if(insert_run_attrs_in_batch) {
	    ...
	    rc = metadata_insert_run_attribute (attribute1_id, new_attr);
	    if (rc != RC_OK) {
    ...
    if(insert_run_attrs_in_batch) {
	    ...
	    rc = metadata_insert_run_attribute (attribute2_id, new_attr);
	    if (rc != RC_OK) {
    ...
    if(insert_run_attrs_in_batch) {
	    ...
	    rc = metadata_insert_run_attribute (attribute3_id, new_attr);
	    if (rc != RC_OK) {
    ...
    if(insert_run_attrs_in_batch) {
	    ...
	    rc = metadata_insert_run_attribute (attribute4_id, new_attr);
	    if (rc != RC_OK) {
    ...
    if(insert_run_attrs_in_batch) {
	    ...
	    rc = metadata_insert_run_attribute_batch (all_run_attrs);
	    if (rc != RC_OK) {
	        error_log << "Error. Was unable to insert run attrs by batch. Proceeding" << endl;
	    }
    }
    else {
	    rc = metadata_insert_run_attribute (attribute5_id, new_attr);
	    if (rc != RC_OK) {
...
int create_timestep_attrs(uint64_t timestep_id, uint64_t timestep_id2, uint64_t txn_id,
                    uint64_t type0_id, uint64_t type1_id )   
{
    int rc;
    ...
    if(insert_timestep_attrs_in_batch) {
	    ...
	    rc = metadata_insert_timestep_attribute (attribute0_id, new_attr);
	    if (rc != RC_OK) {
    ...
    if(insert_timestep_attrs_in_batch) {
	    ...
	    rc = metadata_insert_timestep_attribute (attribute1_id, new_attr);
	    if (rc != RC_OK) {
	        error_log << "Error. Was unable to insert the second new_attr. Proceeding" << endl;
	    }
	    md_log( "Second new_attr id is " + to_string(attribute1_id ) );
    ...
    if(insert_timestep_attrs_in_batch) {
	    ...
	    rc = metadata_insert_timestep_attribute (attribute2_id, new_attr);
	    if (rc != RC_OK) {
	        error_log << "Error. Was unable to insert the third new_attr. Proceeding" << endl;
	    }
	    md_log( "Third new_attr id is " + to_string( attribute2_id) );
    ...
    if(insert_timestep_attrs_in_batch) {
	    ...
	    rc = metadata_insert_timestep_attribute (attribute3_id, new_attr);
	    if (rc != RC_OK) {
	        error_log << "Error. Was unable to insert the Fourth new_attr. Proceeding" << endl;
	    }
	    md_log( "Fourth new_attr id is " + to_string(attribute3_id) );
    ...
    if(insert_timestep_attrs_in_batch) {
	    ...
	    rc = metadata_insert_timestep_attribute (attribute4_id, new_attr);
	    if (rc != RC_OK) {
	        error_log << "Error. Was unable to insert the fifth new_attr. Proceeding" << endl;
	    }
    ...
    if(insert_timestep_attrs_in_batch) {
    	...
    	rc = metadata_insert_timestep_attribute_batch (all_timestep_attrs);
	    if (rc != RC_OK) {
	        error_log << "Error. Was unable to insert the timestep attrs by batch. Proceeding" << endl;
	    }
    }
    else {
	    rc = metadata_insert_timestep_attribute (attribute5_id, new_attr);
	    if (rc != RC_OK) {
	        error_log << "Error. Was unable to insert the Sixth new_attr. Proceeding" << endl;
	    }
...
int create_new_timesteps(uint64_t run_id, uint64_t run_id2, uint64_t timestep_id, uint64_t timestep_id2, 
                    int64_t type_id, uint64_t var_id, uint64_t txn_id) 
{
    ...
    rc = metadata_create_timestep (timestep_id+10, run_id, new_timestep_path, txn_id);
    if (rc != RC_OK) {
    ...
    // rc = metadata_create_timestep (timestep_id+20, run_id, new_timestep_path, txn_id);
    // if (rc != RC_OK) {
    //     error_log << "Error. Was unable to create the first new timestep. Exiting \n";
    //     return RC_ERR;
    // }
    ...
    rc = metadata_create_timestep (timestep_id+30, run_id, new_timestep_path, txn_id);
    if (rc != RC_OK) {
        error_log << "Error. Was unable to create the first new timestep. Exiting \n";
        return RC_ERR;
    }
    ...
    rc = metadata_create_timestep (timestep_id+40, run_id, new_timestep_path, txn_id);
    if (rc != RC_OK) {
        error_log << "Error. Was unable to create the first new timestep. Exiting \n";
        return RC_ERR;
    }
    ...
    rc = metadata_create_timestep (timestep_id+10, run_id2, new_timestep_path, txn_id);
    if (rc != RC_OK) {
    ...
    rc = metadata_create_timestep (timestep_id2+100, run_id, new_timestep_path, txn_id+100);
    if (rc != RC_OK) {
...
int create_new_types(uint64_t run_id, uint64_t run_id2, uint64_t timestep_id, uint64_t type_id2, 

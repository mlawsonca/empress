...
int catalog_all_var_attributes_with_type_var ( md_server server, md_catalog_type_entry type, md_catalog_var_entry var,
    ...
    rc = metadata_catalog_all_var_attributes_with_type_var_by_id(server, var.timestep_id, type_id, var_id, var.txn_id, count, attr_entries);
    if (rc == RC_OK) {
    ...
    rc = metadata_catalog_all_var_attributes_with_type_var_dims_by_id(server, var.timestep_id, type_id, var_id, var.txn_id, num_dims, vect_dims, count, attr_entries);
    if (rc == RC_OK) {
...
int catalog_all_var_attributes_with_type ( md_server server, uint64_t timestep_id, int num_dims, 
    ...
    rc = metadata_catalog_all_var_attributes_with_type_by_id (server, timestep_id, type_id, type.txn_id, count, attr_entries);
    ...
    rc = metadata_catalog_all_var_attributes_with_type_dims_by_id (server, timestep_id, type_id, type.txn_id, num_dims, vect_dims, count, attr_entries);
...
static int catalog_all_var_attributes_with_type_id ( md_server server, uint64_t timestep_id, uint64_t txn_id, uint64_t type_id ) {
    ...
    rc = metadata_catalog_all_var_attributes_with_type_by_id (server, timestep_id, type_id, txn_id, count, attr_entries);
...
/////////////active run testing//////////////////////////////////////////////////////////////////////////////
    ...
    if(my_server_type == SERVER_DEDICATED_D2T_TRANSACTION_MANAGEMENT) {
        ...
        if (rc == RC_OK) {
            ...
            error_log << "Error. Was unable to activate the new run. Continuing \n";
...
/////////////start delete run testing//////////////////////////////////////////////////////////////////////////////
    ...
    if (rc == RC_OK) {
        ...
        error_log << "Error. Problem with activate run attr testing. Continuing \n";
        // return RC_ERR;
    }

    rc = metadata_delete_run_by_id (server, new_run_id);
    if (rc == RC_OK) {
...
static int test_activate_run_attrs ( md_server server, md_catalog_run_entry prev_run, uint64_t run_id, uint64_t type_id ) {
    ...
    if(my_server_type == SERVER_DEDICATED_D2T_TRANSACTION_MANAGEMENT) {
        ...
        if (rc == RC_OK) {
            ...
            error_log << "Error. Was unable to activate the new run attribute. Continuing \n";
            // return RC_ERR;
        }
    }
...
int test_activate_and_delete_timestep ( md_server server, md_catalog_timestep_entry prev_timestep, 
    ...
    if(my_server_type == SERVER_DEDICATED_D2T_TRANSACTION_MANAGEMENT) {
        ...
        rc = metadata_activate_timestep(server, new_timestep.txn_id);
        if (rc == RC_OK) {
            testing_log << "just activated the new timestep  \n";
        }
        else {
            // error_log << "Error. Was unable to activate the new timestep. Exiting \n";
            error_log << "Error. Was unable to activate the new timestep. Continuing \n";
            // return RC_ERR;
        }
...
/////////////start delete timestep testing//////////////////////////////////////////////////////////////////////////////
    ...
    if (rc != RC_OK) {
        ...
        error_log << "Error with the test activate timestep attributes testing. Continuing \n";
        // return rc;
    }
   ...
   //do for each var
    ...
    rc = metadata_delete_timestep_by_id ( server, new_timestep.timestep_id, new_timestep.run_id);
    if (rc != RC_OK) {
...
static int test_activate_timestep_attrs ( md_server server, md_catalog_timestep_entry timestep, uint64_t type_id ) {
    ...
    if(my_server_type == SERVER_DEDICATED_D2T_TRANSACTION_MANAGEMENT) {
        ...
        if (rc == RC_OK) {
            ...
            error_log << "Error. Was unable to activate the new attribute. Continuing \n";
            // return RC_ERR;
        }
    }
    // if(my_server_type  == SERVER_DEDICATED_SQLITE_TRANSACTION_MANAGEMENT_WAL) {
...
int test_activate_and_delete_type ( md_server server, md_catalog_type_entry prev_type, md_catalog_var_attribute_entry prev_attr) {
    ...
    if(my_server_type == SERVER_DEDICATED_D2T_TRANSACTION_MANAGEMENT) {
        ...
        if (rc == RC_OK) {
            ...
            error_log << "Error. Was unable to activate the new type. Continuing \n";
            // return RC_ERR;
        }
    }
...
/////////////active type testing//////////////////////////////////////////////////////////////////////////////
    ...
    // if(my_server_type != SERVER_DEDICATED_SQLITE_TRANSACTION_MANAGEMENT_WAL) {
        ...
        if (rc == RC_OK) {
            ...
            error_log << "Error. Was unable to deactivate the new type. Continuing \n";
...
/////////////start delete var testing//////////////////////////////////////////////////////////////////////////////
    ...
    rc = metadata_delete_type_by_id (server, new_type_id);
    if (rc == RC_OK) {
...
/////////////active var testing//////////////////////////////////////////////////////////////////////////////
    ...
    if(my_server_type == SERVER_DEDICATED_D2T_TRANSACTION_MANAGEMENT) {
        ...
        if (rc == RC_OK) {
            ...
            error_log << "Error. Was unable to activate the new var. Continuning \n";
            // return RC_ERR;
        }
    }
    ...
    // if(my_server_type != SERVER_DEDICATED_SQLITE_TRANSACTION_MANAGEMENT_WAL) {
        ...
        if (rc == RC_OK) {
            ...
            error_log << "Error. Was unable to deactivate the new var. Continuing \n";
            // return RC_ERR;
        }
    ...
    if(my_server_type != SERVER_DEDICATED_D2T_TRANSACTION_MANAGEMENT) {
        //need to put it back since the var attrs require the var to be in the db for cross reference
        rc = metadata_create_var (server, new_var_id, new_var);
        if (rc != RC_OK) {
            error_log << "Error. Was unable to create the new var. Exiting \n";
            return RC_ERR;
...
/////////////start delete var testing//////////////////////////////////////////////////////////////////////////////
    ...
    if (rc == RC_OK) {
        ...
        error_log << "Error. Problem with the activate var attr testing. Continuing \n";
        // return RC_ERR;
    }

    rc = metadata_delete_var_by_id (server, new_var.run_id, new_var.timestep_id, new_var_id);
    if (rc == RC_OK) {
...
int delete_run (md_server server, uint64_t run_id, uint64_t timestep_id, uint64_t txn_id ) {
    ...
    rc = metadata_delete_run_by_id (server, run_id);

...
int test_activate_run ( md_server server, md_catalog_run_entry prev_run, md_catalog_type_entry prev_type, 
    ...
    rc = metadata_activate_run(server, prev_run.txn_id);
    if (rc == RC_OK) {
    ...
    rc = metadata_create_run (server, new_run_id, job_id, prev_run.name, prev_run.txn_id, prev_run.npx, prev_run.npy, prev_run.npz, 
    ...
    new_type.txn_id = prev_run.txn_id;
    new_type.version = 3;
...
static int test_activate_run_attrs ( md_server server, md_catalog_run_entry prev_run, uint64_t run_id, uint64_t type_id ) {
    ...
    new_attr.txn_id = prev_run.txn_id;
    ...
    rc = metadata_activate_run_attribute(server, prev_run.txn_id);
    if (rc == RC_OK) {
    ...
    rc = metadata_catalog_all_run_attributes_in_run ( server, run_id, new_txn_id, count, attr_entries );
    if (rc == RC_OK) {
...
int test_activate_and_delete_timestep ( md_server server, md_catalog_timestep_entry prev_timestep, 
    ...
    uint64_t new_txn_id = prev_timestep.txn_id + 100;
    new_timestep.timestep_id = old_timestep_id + 100;


    rc = metadata_activate_timestep(server, prev_timestep.txn_id);
    if (rc == RC_OK) {
...
/////////////active timestep testing//////////////////////////////////////////////////////////////////////////////
    ...
    //returns information about the timesteps associated with the given txn_id 
    rc = metadata_catalog_timestep (server, new_timestep.run_id, new_txn_id, count, timestep_entries);
    if (rc == RC_OK) {
        md_log( "now the number of timesteps is " + to_string(count) );
...
static int test_activate_timestep_attrs ( md_server server, md_catalog_timestep_entry timestep, uint64_t type_id ) {
    ...
    uint64_t new_attr_id;
    uint64_t old_txn_id = timestep.txn_id;
    uint64_t new_txn_id = old_txn_id + 100;

    md_catalog_timestep_attribute_entry new_attr;
    new_attr.timestep_id = timestep.timestep_id;
    new_attr.type_id = type_id;
    new_attr.txn_id = old_txn_id;
    ...
    rc = metadata_activate_timestep_attribute(server, old_txn_id);
    if (rc == RC_OK) {
    ...
    rc = metadata_catalog_all_timestep_attributes(server, timestep.run_id, new_txn_id, count, attr_entries);
    if (rc == RC_OK) {
    ...
    rc = metadata_catalog_all_timestep_attributes_in_timestep(server, timestep.run_id, timestep.timestep_id, new_txn_id, count, attr_entries);
    if (rc == RC_OK) {
...
int test_activate_and_delete_type ( md_server server, md_catalog_type_entry prev_type, md_catalog_var_attribute_entry prev_attr) {
    ...
    uint64_t new_txn_id = prev_type.txn_id + 100;
    md_catalog_type_entry new_type = prev_type;
    // new_type.txn_id = prev_type.txn_id + 100;
    new_type.version = 3;


    cout << "about to activate type with txn_id: " << new_type.txn_id << endl;
    ...
    cout << "about to create new type with txn_id: " << new_type.txn_id << endl;
    rc = metadata_create_type (server, new_type_id, new_type);
    if (rc != RC_OK) {
        error_log << "Error. Was unable to create the new type. Proceeding" << endl;
    }
...
/////////////active type testing//////////////////////////////////////////////////////////////////////////////
    //returns information about the types associated with the given txn_id 
    rc = metadata_catalog_type (server, new_type.run_id, new_txn_id, count, type_entries);
...
int test_activate_and_delete_var_and_var_attrs ( md_server server, uint64_t type_id, 
    ...
    uint64_t new_var_id;
    uint64_t new_txn_id = prev_var.txn_id + 100;
...
/////////////active var testing//////////////////////////////////////////////////////////////////////////////
    rc = metadata_catalog_var (server, prev_var.run_id, prev_var.timestep_id, new_txn_id, count, var_entries);
...
static int test_activate_var_attrs ( md_server server, uint64_t type_id, uint64_t var_id, 
    ...
    uint64_t old_txn_id = var.txn_id;
    uint64_t new_txn_id = var.txn_id - 100;
    // var.txn_id -= 100;
    ...
    rc = metadata_activate_var_attribute(server, old_txn_id);
    if (rc == RC_OK) {
    ...
    type.txn_id = new_txn_id;
    var.txn_id = new_txn_id;

    rc = catalog_all_var_attributes ( server, var.run_id, var.timestep_id, new_txn_id, num_dims, vect_dims );

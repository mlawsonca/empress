...
int catalog_all_run_attributes ( md_server server, uint64_t run_id, uint64_t type_id, uint64_t txn_id) {
    ...
    //reminder, the run attr for a txn_id 105 will still show up since we are querying uncommitted 
    //(and even if we wanted to view committed only, we can't do this since the uncommitted new attr has already been written)
                    ...
                    uint64_t type0_id, uint64_t type1_id )   
{
    ...
    cout << "create_timestep_attrs for run_id: " << run_id << endl;

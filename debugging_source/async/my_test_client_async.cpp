...
int run_test(uint64_t job_id, int rank, faodel::DirectoryInfo dir, md_server dirman) {
    ...
    //extreme_debug_log << "done with register_objector_funct_write" << endl;
    ...
    //extreme_debug_log << "done with create_timestep" << endl;

  	fut = metadata_checkpoint_database_async (server, new_run.job_id, DB_COPY);
    getAsyncReturnedValue(fut, rc);
    //extreme_debug_log << "done with metadata_checkpoint_database_async" << endl;
...
//TYPE AND ATTRIBUTE TESTING
    ...
    fut = metadata_catalog_type_async (server, run0_id, txn_id);
...
//NEW ATTRIBUTE TESTING
    ...
    fut = metadata_catalog_all_var_attributes_with_dims_async (server, run0_id, timestep0_id, txn_id, temp_new_num_dims, temp_vect_dims);
...
//TESTING VAR SUBSTR OPS
    ...
    fut = metadata_create_var_async (server, new_substr_var);
    getAsyncReturnedValue(fut, rc);
    if (rc != RC_OK) {
    ...
    new_subst_attr.var_id = new_substr_var.var_id;
    ...
    fut = metadata_insert_var_attribute_by_dims_async (server, new_subst_attr);
    getAsyncReturnedValue(fut, new_substr_var_id, rc);
    if (rc != RC_OK) {
    ...
    fut = metadata_create_timestep_async (server, timestep2_id, new_timestep.run_id, timestep2_path, new_timestep.txn_id);
    rc = stoi(fut.get());
    if (rc != RC_OK) {
    ...
    fut = metadata_create_type_async (server, new_type_substr);
    getAsyncReturnedValue(fut, new_type_id_subst, rc);
    if (rc != RC_OK) {
    ...
    fut = metadata_insert_var_attribute_by_dims_async (server, new_subst_attr);
    getAsyncReturnedValue(fut, attribute_id_new, rc);
    if (rc != RC_OK) {
        error_log << "Error. Was unable to insert the new substr attr. Proceeding" << endl;
    }
...
// //and finding certain types for a specific timestep
    ...
    rc = create_timestep_attrs( server, timestep0_id, timestep1_id, txn_id, type0_id, type1_id );

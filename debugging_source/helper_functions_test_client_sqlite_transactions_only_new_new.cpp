...
static int catalog_all_var_attributes_with_type_id ( md_server server, uint64_t run_id, uint64_t timestep_id, uint64_t txn_id, uint64_t type_id );
...
int catalog_all_var_attributes_with_type_var ( md_server server, md_catalog_type_entry type, md_catalog_var_entry var,
    ...
    rc = metadata_catalog_all_var_attributes_with_type_var_by_id(server, var.run_id, var.timestep_id, type_id, var_id, var.txn_id, count, attr_entries);
    if (rc == RC_OK) {
    ...
    rc = metadata_catalog_all_var_attributes_with_type_var_by_name_ver(server, var.run_id, var.timestep_id, to_upper(type.name), type.version, 
    ...
    // rc = metadata_catalog_all_var_attributes_with_type_var_range(server, var.timestep_id, type_id, var_id, var.txn_id, 
    ...
    // rc = metadata_catalog_all_var_attributes_with_type_var_above_max(server, var.timestep_id, type_id, var_id, var.txn_id, 
    ...
    // rc = metadata_catalog_all_var_attributes_with_type_var_below_min(server, var.timestep_id, type_id, var_id, var.txn_id, 
    ...
    rc = metadata_catalog_all_var_attributes_with_type_var_dims_by_id(server, var.run_id, var.timestep_id, type_id, var_id, var.txn_id, num_dims, vect_dims, count, attr_entries);
    if (rc == RC_OK) {
    ...
    rc = metadata_catalog_all_var_attributes_with_type_var_dims_by_name_ver(server, var.run_id, var.timestep_id, to_upper(type.name), type.version, 
    ...
    // rc = metadata_catalog_all_var_attributes_with_type_var_dims_range(server, var.timestep_id, type_id, var_id, var.txn_id, 
    ...
    // rc = metadata_catalog_all_var_attributes_with_type_var_dims_above_max(server, var.timestep_id, type_id, var_id, var.txn_id, 
    ...
    // rc = metadata_catalog_all_var_attributes_with_type_var_dims_below_min(server, var.timestep_id, type_id, var_id, var.txn_id, 
...
int catalog_all_var_attributes ( md_server server, uint64_t run_id, uint64_t timestep_id, uint64_t txn_id, int num_dims, 
    ...
    rc = metadata_catalog_all_var_attributes_with_dims(server, run_id, timestep_id, txn_id, num_dims, vect_dims, count, attr_entries);
...
int catalog_all_var_attributes_with_type ( md_server server, uint64_t timestep_id, int num_dims, 
    ...
    rc = metadata_catalog_all_var_attributes_with_type_by_id (server, type.run_id, timestep_id, type_id, type.txn_id, count, attr_entries);
    ...
    rc = metadata_catalog_all_var_attributes_with_type_by_name_ver (server, type.run_id, timestep_id, to_upper(type.name), type.version, type.txn_id, count, attr_entries);
    ...
    rc = metadata_catalog_all_var_attributes_with_type_dims_by_id (server, type.run_id, timestep_id, type_id, type.txn_id, num_dims, vect_dims, count, attr_entries);
    ...
    rc = metadata_catalog_all_var_attributes_with_type_dims_by_name_ver(server, type.run_id, timestep_id, to_upper(type.name), type.version, type.txn_id, num_dims, vect_dims, count, attr_entries);
...
static int catalog_all_var_attributes_with_type_id ( md_server server, uint64_t run_id, uint64_t timestep_id, uint64_t txn_id, uint64_t type_id ) {
    ...
    rc = metadata_catalog_all_var_attributes_with_type_by_id (server, run_id, timestep_id, type_id, txn_id, count, attr_entries);
...
int catalog_all_var_attributes_with_var ( md_server server, md_catalog_var_entry var, uint64_t var_id, int num_dims, vector<md_dim_bounds> vect_dims ) {
    ...
    rc = metadata_catalog_all_var_attributes_with_var_by_id (server, var.run_id, var.timestep_id, var_id, var.txn_id, count, attr_entries);
    ...
    rc = metadata_catalog_all_var_attributes_with_var_by_name_ver (server, var.run_id, var.timestep_id, to_upper(var.name), var.version, var.txn_id, count, attr_entries);
    if (rc == RC_OK) {
    ...
    rc = metadata_catalog_all_var_attributes_with_var_dims_by_id (server, var.run_id, var.timestep_id, var_id, var.txn_id, num_dims, vect_dims, count, attr_entries);
    if (rc == RC_OK) {
    ...
    rc = metadata_catalog_all_var_attributes_with_var_dims_by_name_ver(server, var.run_id, var.timestep_id, to_upper(var.name), var.version, var.txn_id, num_dims, vect_dims, count, attr_entries);
    if (rc == RC_OK) {
...
//     rc = metadata_catalog_all_var_attributes_with_var_by_id (server, run_id, timestep_id, var_id, txn_id, count, attr_entries);
...
int test_activate_run ( md_server server, md_catalog_run_entry prev_run, md_catalog_type_entry prev_type, 
    ...
    rc = metadata_create_run (server, new_run_id, job_id, prev_run.name, new_txn_id, prev_run.npx, prev_run.npy, prev_run.npz, 
...
/////////////active run testing//////////////////////////////////////////////////////////////////////////////
    ...
    rc = metadata_activate_run(server, new_txn_id);
    if (rc == RC_OK) {
    ...
    rc = metadata_catalog_run (server, prev_run.txn_id, count, run_entries);
    ...
    rc = metadata_processing_run(server, new_txn_id);
   ...
   //returns information about the types associated with the given txn_id 
    rc = metadata_catalog_type (server, new_run_id, new_txn_id, count, type_entries);
    if (rc == RC_OK) {
...
/////////////start delete run testing//////////////////////////////////////////////////////////////////////////////
    ...
    rc = metadata_delete_run_by_id (server, new_run_id, prev_run.txn_id);
    if (rc == RC_OK) {
    ...
    //should be empty now
    rc = metadata_catalog_run (server, new_txn_id, count, run_entries);
    if (rc == RC_OK) {
   ...
   //returns information about the types associated with the given txn_id 
    ...
    //should be empty now
    rc = metadata_catalog_all_run_attributes ( server, new_txn_id, count, attr_entries );
    if (rc == RC_OK) {
    ...
    rc = metadata_catalog_all_run_attributes_in_run ( server, new_run_id, new_txn_id, count, attr_entries );
    if (rc == RC_OK) {
...
static int test_activate_run_attrs ( md_server server, md_catalog_run_entry prev_run, uint64_t run_id, uint64_t type_id ) {
    ...
    rc = metadata_insert_run_attribute (server, new_attr_id, new_attr);
    if (rc != RC_OK) {
    ...
    rc = metadata_activate_run_attribute(server, new_txn_id);
    if (rc == RC_OK) {
    ...
    rc = metadata_catalog_all_run_attributes ( server, prev_run.txn_id, count, attr_entries );
    if (rc == RC_OK) {
    ...
    rc = metadata_catalog_all_run_attributes_in_run ( server, run_id, prev_run.txn_id, count, attr_entries );
    if (rc == RC_OK) {
    ...
    rc = metadata_processing_run_attribute(server, new_txn_id);
...
int test_activate_and_delete_timestep ( md_server server, md_catalog_timestep_entry prev_timestep, 
    ...
    rc = metadata_create_timestep (server, new_timestep.timestep_id, new_timestep.run_id, new_timestep.path, new_timestep.txn_id);
    if (rc != RC_OK) {
...
/////////////active timestep testing//////////////////////////////////////////////////////////////////////////////
    ...
    rc = metadata_catalog_timestep (server, new_timestep.run_id, prev_timestep.txn_id, count, timestep_entries);
    if (rc == RC_OK) {
    ...
    rc = metadata_processing_timestep(server, new_timestep.txn_id);
...
/////////////start delete timestep testing//////////////////////////////////////////////////////////////////////////////
    ...
    // prev_var.var_id = 2;
    rc = metadata_create_var (server, new_var_id, prev_var);
    if (rc != RC_OK) {
   ...
   //do for each var
    ...
    rc = metadata_catalog_var (server, new_timestep.run_id, new_timestep.timestep_id, new_timestep.txn_id, count, var_entries);
    ...
    rc = metadata_insert_var_attribute_by_dims (server, new_attr_id, prev_attr);
    if (rc != RC_OK) {
    ...
    rc = metadata_delete_timestep_by_id ( server, new_timestep.timestep_id, new_timestep.run_id, new_timestep.txn_id);
    if (rc != RC_OK) {
...
static int test_activate_timestep_attrs ( md_server server, md_catalog_timestep_entry timestep, uint64_t type_id ) {
    ...
    new_attr.run_id = timestep.run_id;
    ...
    rc = metadata_insert_timestep_attribute (server, new_attr_id, new_attr);
    if (rc != RC_OK) {
    ...
    rc = metadata_activate_timestep_attribute(server, new_txn_id);
    if (rc == RC_OK) {
    ...
    rc = metadata_catalog_all_timestep_attributes(server, timestep.run_id, timestep.txn_id, count, attr_entries);
    if (rc == RC_OK) {
    ...
    rc = metadata_catalog_all_timestep_attributes_in_timestep(server, timestep.run_id, timestep.timestep_id, timestep.txn_id, count, attr_entries);
    if (rc == RC_OK) {
    ...
    rc = metadata_processing_timestep_attribute(server, new_txn_id);
    if (rc == RC_OK) {
...
/////////////active type testing//////////////////////////////////////////////////////////////////////////////
    ...
    rc = metadata_activate_type(server, new_type.txn_id);
    if (rc == RC_OK) {
    ...
    //returns information about the types associated with the given txn_id 
    rc = metadata_catalog_type (server, new_type.run_id, prev_type.txn_id, count, type_entries);
    ...
    rc = metadata_processing_type(server, new_type.txn_id);
...
/////////////start delete var testing//////////////////////////////////////////////////////////////////////////////
    ...
    rc = catalog_all_var_attributes_with_type_id ( server, new_type.run_id, prev_attr.timestep_id, new_type.txn_id, new_type_id );
    if (rc != RC_OK) {
        error_log << "Error. Was unable to catalog all var attributes with_type_id " << new_type_id << ". Proceeding" << endl;
    }


    rc = metadata_delete_type_by_id (server, new_type_id, new_type.txn_id);
    if (rc == RC_OK) {
    ...
    // rc = metadata_delete_type_by_name_ver (server, new_type.run_id, new_type.name, new_type.version);
    // if (rc == RC_OK) {
    ...
    // rc = metadata_catalog_all_var_attributes ( server, prev_type.run_id, prev_attr.timestep_id, prev_type.txn_id, count, attr_entries);
    // if (rc == RC_OK) {
...
/////////////active var testing//////////////////////////////////////////////////////////////////////////////
    ...
    rc = metadata_activate_var(server, new_var.txn_id);
    if (rc == RC_OK) {
    ...
    rc = metadata_catalog_var (server, prev_var.run_id, prev_var.timestep_id, prev_var.txn_id, count, var_entries);
    ...
    rc = metadata_processing_var(server, new_var.txn_id);
...
/////////////start delete var testing//////////////////////////////////////////////////////////////////////////////
    ...
    rc = test_activate_var_attrs ( server, type_id, new_var_id, prev_type, new_var, num_dims, vect_dims);
    if (rc == RC_OK) {
    ...
    rc = metadata_delete_var_by_id (server, new_var.run_id, new_var.timestep_id, new_var_id, new_var.txn_id);
    if (rc == RC_OK) {
    ...
    // rc = metadata_delete_var_by_name_path_ver (server, new_var.run_id, new_var.timestep_id, new_var.name, new_var.path, new_var.version);
    // if (rc == RC_OK) {
    ...
    rc = metadata_catalog_var (server, new_var.run_id, new_var.timestep_id, prev_var.txn_id, count, var_entries);
    if (rc == RC_OK) {
        md_log( "now the number of vars is " + to_string(count) );
    ...
    //should be empty now
    rc = metadata_catalog_var (server, new_var.run_id, new_var.timestep_id, new_var.txn_id, count, var_entries);
    if (rc == RC_OK) {
        md_log( "now the number of vars is " + to_string(count) );
...
static int test_activate_var_attrs ( md_server server, uint64_t type_id, uint64_t var_id, 
    ...
    rc = metadata_insert_var_attribute_by_dims (server, new_attr_id, new_attr);
    if (rc != RC_OK) {
        error_log << "Error. Was unable to insert the new var attribute. Proceeding" << endl;
    }
    md_log( "New var attribute id is " + to_string(new_attr_id) );

    rc = metadata_activate_var_attribute(server, new_txn_id);
    if (rc == RC_OK) {
    ...
    rc = metadata_processing_var_attribute(server, new_txn_id);
...
int delete_run (md_server server, uint64_t run_id, uint64_t timestep_id, uint64_t txn_id ) {
    ...
    // rc = metadata_delete_run_by_name_path_timestep (server, run0_name, run0_timestep);
    rc = metadata_delete_run_by_id (server, run_id, txn_id);
    ...
    //Should be empty now 
    rc = metadata_catalog_all_run_attributes ( server, txn_id, count, run_attr_entries );
    if (rc == RC_OK) {
    ...
    rc = metadata_catalog_all_run_attributes_in_run ( server, run_id, txn_id, count, run_attr_entries );
    if (rc == RC_OK) {
    ...
    //Should be empty now 
    rc = metadata_catalog_all_timestep_attributes ( server, run_id, txn_id, count, timestep_attr_entries );
    if (rc == RC_OK) {
    ...
    rc = metadata_catalog_all_timestep_attributes_in_timestep ( server, run_id, timestep_id, txn_id, count, timestep_attr_entries );
    if (rc == RC_OK) {
    ...
    //Should be empty now 
    rc = metadata_catalog_all_var_attributes (server, run_id, timestep_id, txn_id, count, var_attr_entries);
    if (rc == RC_OK) {
...
{    
    ...
    if(insert_types_in_batch) {
        ...
        // rc = metadata_create_type_batch (server, all_types);
        // rc = metadata_create_type_batch (server, all_type_ids, all_types);
        rc = metadata_create_type_batch (server, type0_id, all_types);
        if (rc != RC_OK) {
        ...
        rc = metadata_create_type (server, type1_id, new_type2);
        if (rc != RC_OK) {
    ...
    if (rc == RC_OK) {
        testing_log << "just activated the run  \n";
    }
    else {
        error_log << "Error. Was unable to activate the run. Exiting \n";
        return RC_ERR;
    }

    rc = metadata_activate_type(server, run.txn_id);
    if (rc == RC_OK) {
        testing_log << "just activated the type  \n";
    }
    else {
        error_log << "Error. Was unable to activate the type. Exiting \n";
        return RC_ERR;
    }
    return rc;
}
                    ...
                    md_catalog_var_entry &new_var, md_catalog_var_entry &new_var2, md_catalog_var_attribute_entry &new_attr )   
{
   ...
   //do for each var
    ...
    if(insert_vars_in_batch) {
        ...
        rc = metadata_create_var (server, var1_id, new_var2);
        if (rc != RC_OK) {
...
//TYPE AND ATTRIBUTE SETUP
    ...
    if(insert_attrs_in_batch) {
        ...
        rc = metadata_insert_var_attribute_by_dims(server, attribute1_id, new_attr);
        if (rc != RC_OK) {
    ...
    if (insert_attrs_in_batch) {
        ...
        rc = metadata_insert_var_attribute_by_dims(server, attribute2_id, new_attr);
        if (rc != RC_OK) {
    ...
    if(insert_attrs_in_batch) {
        ...
        rc = metadata_insert_var_attribute_by_dims(server, attribute3_id, new_attr);
        if (rc != RC_OK) {
    ...
    if(insert_attrs_in_batch) {
        ...
        rc = metadata_insert_var_attribute_by_dims(server, attribute4_id, new_attr);
        if (rc != RC_OK) {
    ...
    rc = metadata_activate_timestep(server, txn_id);
    if (rc == RC_OK) {
        testing_log << "just activated the timestep  \n";
    }
    else {
        error_log << "Error. Was unable to activate the timestep. Exiting \n";
        return RC_ERR;
    }

    rc = metadata_activate_var(server, txn_id);
    if (rc == RC_OK) {
        testing_log << "just activated the var  \n";
    }
    else {
        error_log << "Error. Was unable to activate the var. Exiting \n";
        return RC_ERR;
    }

    rc = metadata_activate_var_attribute(server, txn_id);
    if (rc == RC_OK) {
        testing_log << "just activated the var attribute  \n";
    }
    else {
        error_log << "Error. Was unable to activate the var attribute. Exiting \n";
        return RC_ERR;
    }

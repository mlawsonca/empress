...
struct db_conns {
    sqlite3 *db;    
    // sqlite3 *db_read; //commited db
    bool transaction_open = false;
    deque<md_catalog_type> savepts_stack;
    // bool run_savept;
    // bool timestep_savept;
    // bool var_savept;
    // bool run_attr_savept;
    // bool timestep_attr_savept;
    // bool var_attr_savept; 
    // bool type_savept; 
    bool savepts_active[7] = {false, false, false, false, false, false, false};


    // db_conns(sqlite3 *my_db, sqlite3 *my_db_read) {
    db_conns(sqlite3 *my_db) {
        db = my_db;
        // db_read = my_db_read;
    }
    ...
    // db_conns(sqlite3 *my_db, bool trans_open) {
    //     db = my_db;
    //     transaction_open = trans_open;
    // }
};

unordered_map<uint64_t,db_conns> job_id_to_database_connections{};
...
void close_database(uint64_t job_id);
static int metadata_database_init(sqlite3 **db, bool load_db, uint64_t job_id);
static int create_reading_indices(sqlite3 *db, uint64_t job_id);
...
int main (int argc, char **argv)
{
    ...
    // sqlite3 *main_db = NULL, *main_db_read = NULL;
    ...
    if (rc != SQLITE_OK)    {
        fprintf (stdout, "Error setting sqlite3_config. Line: %d \n", __LINE__);
        goto cleanup;
    }
    ...
    rc = metadata_database_init(&main_db, load_db, job_id);
    if (rc != RC_OK) {
...
cleanup:
    main_db = job_id_to_database_connections[job_id].db; //just in case it's been changed
    ...
    //job_id_to_database_connections.erase(job_id);    
    //       close_database(job_id);
    //       goto cleanup;
    //   }
    // }

    //debug_log << "am closing the db at the end of the run" << endl;
    close_database(job_id);
    ...
    if(index_type == WRITE_DELAYED_INDEX && db_output_is_whole()) {
        ...
        rc = create_reading_indices(main_db, job_id);
    }
        ...
        if(rc != RC_OK) {
            error_log << "error with db_compact_and_output" << endl;
        }
    }

    //debug_log << "about to output timing" << endl;
    if(output_timing) {
...
// static int metadata_database_init(sqlite3 **db, sqlite3 **db_read, bool load_db, uint64_t job_id)
static int metadata_database_init(sqlite3 **db, bool load_db, uint64_t job_id)
{
    //setup the database
    int rc;
    char  *ErrMsg = NULL;
    // extreme_debug_log << "metadata_database_init: beginning *db: " << *db << endl;
    ...
    //debug_log << "opening: " << db_name << endl;
    if (rc != SQLITE_OK)
    {   
        fprintf (stderr, "Can't open database: %s\n", sqlite3_errmsg (*db));     
        sqlite3_close (*db);
    ...
    // rc = sqlite3_open_v2(db_name.c_str(), db_read, SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE | SQLITE_OPEN_URI, NULL);
    // //debug_log << "opening for read: " << db_name << endl;
    // if (rc != SQLITE_OK)
    // {   
    //     fprintf (stderr, "Can't open database: %s\n", sqlite3_errmsg (*db_read));     
    ...
    // job_id_to_database_connections[job_id] = db_conns(*db, *db_read);
    job_id_to_database_connections[job_id] = db_conns(*db);
    //debug_log << "initting job_id: " << job_id << " to " << *db << endl;
    ...
    if(load_db) {
        ...
        if(index_type != WRITE_DELAYED_INDEX) {
            rc = create_reading_indices(*db, job_id);
            if(rc != RC_OK) {
                close_database(job_id);
            }
        }
    ...
    // extreme_debug_lig << "metadata_database_init: end *db: " << *db << endl;
...
// {
//     sqlite3 *db_read;
//     return metadata_database_init(db, &db_read, load_db, job_id);
...
int db_checkpoint_copy(uint64_t job_id)
{
    ...
    sqlite3 *db = job_id_to_database_connections[job_id].db;
    ...
    if(rc != RC_OK) {
        error_log << "db_checkpoint_copy: error with db_copy. rc: " << rc << endl;
        return rc;
    }
...
static int db_reset(sqlite3 *db, uint64_t job_id)
{
 ...
 // const char * tail = NULL;
    ...
    //debug_log << "am closing the db for db reset" << endl;
    close_database(job_id);

    rc = metadata_database_init(&db, false, job_id);
    if(rc != RC_OK) {
...
static int db_compact_and_output(uint64_t job_id)
{
    ...
    // }
    //debug_log << "am closing the single db for compact" << endl;
    rc = sqlite3_close(db);
    if (rc != SQLITE_OK)
    {
        fprintf (stderr, "Error db_compact_and_output: Line: %d SQL error (%d): %s\n", __LINE__, rc, sqlite3_errmsg (db));
    }
...
int add_savepoint(sqlite3 *db, uint64_t job_id, md_catalog_type catalog_type) {
    int rc;
    char * ErrMsg = NULL;
    string savepoint_name;
    //extreme_debug_log << "about to get savept_name" << endl;
    switch(catalog_type) {
        case(RUN_CATALOG) : {
            savepoint_name = "savepoint run;";
            break;
        };
        case(TIMESTEP_CATALOG) : {
            savepoint_name = "savepoint timestep;";
            break;
        };
        case(VAR_CATALOG) : {
            savepoint_name = "savepoint var;";
            break;
        };
        case(TYPE_CATALOG) : {
            savepoint_name = "savepoint type;";
            break;
        };
        case(RUN_ATTR_CATALOG) : {
            savepoint_name = "savepoint run_attribute;";
            break;
        };
        case(TIMESTEP_ATTR_CATALOG) : {
            savepoint_name = "savepoint timestep_attribute;";
            break;
        };
        case(VAR_ATTR_CATALOG) : {
            savepoint_name = "savepoint var_attribute;";
            break;
        };
    }
    //extreme_debug_log << "about to add savept for catalog_type: " << catalog_type << endl;
    //extreme_debug_log << "executing command: " << savepoint_name.c_str() << endl;

    rc = sqlite3_exec (db, savepoint_name.c_str(), callback, 0, &ErrMsg);
    if (rc != SQLITE_OK)
    {
        fprintf (stderr, "Error add_savepoint: Line: %d SQL error: %s\n", __LINE__, ErrMsg);
        sqlite3_free (ErrMsg);
        close_database(job_id);
    }
    //extreme_debug_log << "done with add savept for catalog_type: " << catalog_type << endl;
    return rc;
}


sqlite3 * get_database_for_write(uint64_t job_id, md_catalog_type catalog_type) {
    int rc;
    char * ErrMsg = NULL;
    sqlite3 *db;

    std::unordered_map<uint64_t,db_conns>::iterator got = job_id_to_database_connections.find (job_id);

    if ( got == job_id_to_database_connections.end() ) {
        //debug_log << "get_database_for_write: could not find database: " << job_id << " so am adding it" << endl;
    //not found
        metadata_database_init(&db, false, job_id);
        ...
        // add_savepoint(db, job_id, catalog_type);
        //extreme_debug_log << "done with add_savepoint after first creating the db" << endl;
        //extreme_debug_log << "just created new db in get_database_for_write" << endl;

        db_conns *my_conns = &job_id_to_database_connections[job_id];
        my_conns->transaction_open = true;
        my_conns->savepts_active[catalog_type] = true;
        my_conns->savepts_stack.push_back(catalog_type);

    }
    else { //found
        db = (got -> second).db;
        //debug_log << "get_database_for_write: found database: " << job_id << " addr: " << db << endl;

        if(!(got -> second).transaction_open) {
            //debug_log <<  "about to begin transaction" << endl;
             rc = sqlite3_exec (db, "begin;", callback, 0, &ErrMsg);
            if (rc != SQLITE_OK)
            {
                fprintf (stderr, "Error end query get_database_for_write: Line: %d SQL error: %s\n", __LINE__, ErrMsg);
        ...
        //debug_log << "setting savepts[" << catalog_type << "]=true" << endl;
        if(!(got -> second).savepts_active[catalog_type]) {
            //debug_log << "adding " << catalog_type << " to the savept stack" << endl;
            rc = add_savepoint(db, job_id, catalog_type);
            if (rc != SQLITE_OK)
            {
                return NULL;
            }
            //extreme_debug_log << "get_database_for_write: done with add_savepoint" << endl;
            (got -> second).savepts_stack.push_back(catalog_type);
            //extreme_debug_log << "just added savept onto the stack" << endl;
            (got -> second).savepts_active[catalog_type] = true;
            //extreme_debug_log << "just turned the savept to active" << endl;
        }
        //debug_log << "found database " << job_id << endl;
    }


    //debug_log << "just set " << catalog_type << " to true" << endl;

    // for(int i = 0; i < 7; i++) {
    //     //debug_log << "db_conns[" << i << "]: " << job_id_to_database_connections[job_id].savepts[i] << endl;
    // }
    // switch(md_catalog_type) {
    //     case TIMESTEP_CATALOG : {
    //         (got -> second).timestep_savept = true;
    //         break;
    //     }
    //     case RUN_CATALOG : {
    //         (got -> second).run_savept = true;;
    //         break;
    //     }
    //     case VAR_CATALOG : {
    //         (got -> second).var_savept = true;;
    //         break;
    //     }
    //     case TYPE_CATALOG : {
    //         (got -> second).type_savept = true;;
    //         break;
    //     }
    //     case RUN_ATTR_CATALOG : {
    //         (got -> second).run_attr_savept = true;;
    //         break;
    //     }
    //     case TIMESTEP_ATTR_CATALOG : {
    //         (got -> second).timestep_attr_savept = true;;
    //         break;
    //     }
    //     case VAR_ATTR_CATALOG : {
    //         (got -> second).var_attr_savept = true;;
    //         break;
    //     }
    // }   
    return db;
    // return job_id_to_database_connections[job_id];
}


sqlite3 * get_database(uint64_t job_id, md_query_type query_type, bool &trans_active) {
    //debug_log << "job_id_to_database_connections.size(): " << job_id_to_database_connections.size() << endl;
    int rc;
    char * ErrMsg = NULL;
    // sqlite3 *db, *db_read;
    sqlite3 *db;
    ...
    std::unordered_map<uint64_t,db_conns>::iterator got = job_id_to_database_connections.find (job_id);
    if ( got == job_id_to_database_connections.end() ) {
        //debug_log << "get_database: could not find database: " << job_id << " so am adding it" << endl;
    //not found
        // metadata_database_init(&db, &db_read, false, job_id);
        metadata_database_init(&db, false, job_id);
    }
    else { //found
        //debug_log << "found database " << job_id << endl;
        db = (got -> second).db;
        // db_read = (got -> second).db_read;
    }
    //only want to see the committed transactions
    if(query_type == COMMITTED) {
        //debug_log << "get_database: found database for committed: " << job_id << " addr: " << db << endl;
        //debug_log << "returning db for committed read" << endl;
        // if((got -> second).transaction_open) {
        //     //need to relinquish the exclusive lock so the read can be performed
        //     rc = sqlite3_exec (db, "end;", callback, 0, &ErrMsg);
        //     if (rc != SQLITE_OK)
        //     {
        ...
        // }
        trans_active = (got -> second).transaction_open;
        // return db_read;
        return db;
    }
    else { //want to see ongoing transactions in addition to the committed
        //debug_log << "get_database: found database for uncommitted: " << job_id << " addr: " << db << endl;
        //debug_log << "returning db for committed and uncommitted read" << endl;
        trans_active = false;
        return db;
    }

    // return job_id_to_database_connections[txn_id];
}

sqlite3 * get_database_for_transaction_end(uint64_t job_id, bool &trans_active) {
    //committing or aborting the transaction closes the transaction and removes all savepoints
    db_conns *my_conns = &job_id_to_database_connections[job_id];
    trans_active = my_conns->transaction_open;
    my_conns->transaction_open = false;
    for(int i = 0; i < 7; i++) {
        my_conns->savepts_active[i] = false;
    }
    my_conns->savepts_stack.clear();
    //debug_log << "get_database_for_transaction_end: clearing all savepts" << endl;

    return my_conns->db;
}

//used by activate and processing so in either case, any existing savepts will be removed
sqlite3 * get_database_and_status(uint64_t job_id, md_catalog_type catalog_type, bool &savept_active) {
    db_conns *my_conns = &job_id_to_database_connections[job_id];

    //debug_log << "in get_database_and_status" << endl;
    savept_active = my_conns->savepts_active[catalog_type];
    //debug_log << "setting savepts_active[" << catalog_type << "] to: " << savept_active << endl;

    if(savept_active) {
        md_catalog_type next_type;
        do {
            next_type = my_conns->savepts_stack.back();
            my_conns->savepts_active[next_type] = false;
            my_conns->savepts_stack.pop_back();
            //debug_log << "popping savepoint " << next_type << " off the stack" << endl;
        } while (next_type != catalog_type); 
    }  

    //debug_log << "setting savepts[" << catalog_type << "]=false" << endl;
    // switch(md_catalog_type) {
    //     case TIMESTEP_CATALOG : {
    //         savept_active = my_conns->timestep_savept;
    //         my_conns->timestep_savept = false;
    //         break;
    //     }
    //     case RUN_CATALOG : {
    //         savept_active = my_conns->run_savept;
    //         my_conns->run_savept = false;
    //         break;
    //     }
    //     case VAR_CATALOG : {
    //         savept_active = my_conns->var_savept;
    //         my_conns->var_savept = false;
    //         break;
    //     }
    //     case TYPE_CATALOG : {
    //         savept_active = my_conns->type_savept;
    //         my_conns->type_savept = false;
    //         break;
    //     }
    //     case RUN_ATTR_CATALOG : {
    //         savept_active = my_conns->run_attr_savept;
    //         my_conns->run_attr_savept = false;
    //         break;
    //     }
    //     case TIMESTEP_ATTR_CATALOG : {
    //         savept_active = my_conns->timestep_attr_savept;
    //         my_conns->timestep_attr_savept = false;
    //         break;
    //     }
    //     case VAR_ATTR_CATALOG : {
    //         savept_active = my_conns->var_attr_savept;
    //         my_conns->var_attr_savept = false;
    //         break;
    //     }
    // }
    //debug_log << "returning db for job_id: " << job_id << " addr: " << job_id_to_database_connections[job_id].db << endl;

   return job_id_to_database_connections[job_id].db;
}


sqlite3 * get_database(uint64_t job_id) {
    //debug_log << "get_database: returning db for job_id: " << job_id << " addr: " << job_id_to_database_connections[job_id].db << endl;
   return job_id_to_database_connections[job_id].db;
}
...
void close_database(uint64_t job_id) {
    int rc;
    //debug_log << "am closing the db: " << job_id << endl;
    db_conns my_db_conns = job_id_to_database_connections[job_id];
    //debug_log << "about to close write db" << endl;
    rc = sqlite3_close (my_db_conns.db);
    if (rc != SQLITE_OK)
    {
        fprintf (stderr, "Error close_database: Line: %d SQL error (%d): %s\n", __LINE__, rc, sqlite3_errmsg (my_db_conns.db));
    }
    //debug_log << "about to close read db" << endl;
    // rc = sqlite3_close (my_db_conns.db_read);
    // if (rc != SQLITE_OK)
    // {
    //     fprintf (stderr, "Error close_database: Line: %d SQL error (%d): %s\n", __LINE__, rc, sqlite3_errmsg (my_db_conns.db_read));
    // }
    job_id_to_database_connections.erase(job_id);
}

...
int md_insert_var_attribute_by_dims_stub_rtree (const md_insert_var_attribute_by_dims_args &args, uint64_t &attribute_id);
...
WaitingType OpInsertVarAttributeByDimsMeta::handleMessage(bool rdma, message_t *incoming_msg) {
    ...
    if(index_type == INDEX_RTREE) {
        rc = md_insert_var_attribute_by_dims_stub_rtree(sql_args, attribute_id);
    }
    else {
        rc = md_insert_var_attribute_by_dims_stub(sql_args, attribute_id);
    }
...
int md_insert_var_attribute_by_dims_stub_rtree(const md_insert_var_attribute_by_dims_args &args,
                          uint64_t &attribute_id)

{
    ...
    rc = sqlite3_prepare_v2 (db, "insert into var_attribute_dims (var_attr_id, d0_min, d0_max, d1_min, d1_max, d2_min, d2_max) values (?, ?, ?, ?, ?, ?, ?)", -1, &stmt, &tail_index);
    if (rc != SQLITE_OK)
    {
        fprintf (stderr, "Error at start of insert_var_attribute_stub: Line: %d SQL error: %s\n", __LINE__, sqlite3_errmsg (db));
        sqlite3_close (db);
    ...
    rc = sqlite3_bind_int64 (stmt, 1, attribute_id); //assert (rc == SQLITE_OK);
    while(i < args.num_dims) {
        rc = sqlite3_bind_double (stmt, 2 + i * 2, args.dims.at(i).min); //assert (rc == SQLITE_OK);
        rc = sqlite3_bind_double (stmt, 3 + i * 2, args.dims.at(i).max); //assert (rc == SQLITE_OK);
        i++;
    }

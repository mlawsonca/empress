...
static int get_matching_attribute_count (const md_catalog_all_var_attributes_with_type_var_substr_dims_args &args, uint32_t &count)
{
    ...
    query =  "select count (*) from var_attribute_catalog vac "
        "inner join type_catalog tc on vac.type_id = tc.id "
        "inner join var_catalog vc on tc.run_id = vc.run_id and vac.timestep_id = vc.timestep_id and vac.var_id = vc.id "            "inner join var_attribute_dims vad on vac.id = vad.var_attr_id "
        "where (vac.txn_id = ? or vac.active = 1) "
        "and tc.id = ? "
        "and tc.run_id = ? "
        "and vc.name like ? "

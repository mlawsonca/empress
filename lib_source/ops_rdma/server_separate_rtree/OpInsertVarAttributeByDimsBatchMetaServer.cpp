...
int md_insert_var_attribute_by_dims_batch_stub_rtree (const vector<md_insert_var_attribute_by_dims_args> &all_args);
...
WaitingType OpInsertVarAttributeByDimsBatchMeta::handleMessage(bool rdma, message_t *incoming_msg) {
    ...
    if(index_type == INDEX_RTREE) {
        rc = md_insert_var_attribute_by_dims_batch_stub_rtree(sql_args);
    }
    else {
        rc = md_insert_var_attribute_by_dims_batch_stub(sql_args);
    }
    add_timing_point(OP_INSERT_VAR_ATTRIBUTE_BY_DIMS_BATCH_MD_INSERT_VAR_ATTRIBUTE_BY_DIMS_BATCH_STUB);
...
int md_insert_var_attribute_by_dims_batch_stub_rtree (const vector<md_insert_var_attribute_by_dims_args> &all_args)
{
    ...
    if(all_args.size() > 0) {
        ...
        rc = sqlite3_prepare_v2 (db, "insert into var_attribute_catalog (id, timestep_id, type_id, var_id, active, txn_id, num_dims, data_type, data) values (?, ?, ?, ?, ?, ?, ?, ?, ?)", -1, &stmt, &tail_index);
        if (rc != SQLITE_OK)
        {
            fprintf (stderr, "Error at start of insert_var_attribute_stub: Line: %d SQL error: %s\n", __LINE__, sqlite3_errmsg (db));
            sqlite3_close (db);
        ...
        rc = sqlite3_prepare_v2 (db, "insert into var_attribute_dims (var_attr_id, d0_min, d0_max, d1_min, d1_max, d2_min, d2_max) values (?, ?, ?, ?, ?, ?, ?)", -1, &stmt2, &tail_index2);
        if (rc != SQLITE_OK)
        {
            fprintf (stderr, "Error at start of insert_var_attribute_stub: Line: %d SQL error: %s\n", __LINE__, sqlite3_errmsg (db));
            sqlite3_close (db);
    ...
    for(int arg_indx = 0; arg_indx < all_args.size(); arg_indx++) {
        ...
        rc = sqlite3_bind_int64 (stmt, 6, args.txn_id); //assert (rc == SQLITE_OK);
        rc = sqlite3_bind_int (stmt, 7, args.num_dims); //assert (rc == SQLITE_OK);
        rc = sqlite3_bind_int (stmt, 8, args.data_type); //assert (rc == SQLITE_OK);
        ...
        switch(args.data_type) {
            ...
            case ATTR_DATA_TYPE_INT : {
                long long deserialized_int = stoll(args.data);
                rc = sqlite3_bind_int64 (stmt, 9, deserialized_int); 
                break;
            }
            case ATTR_DATA_TYPE_REAL : {
                long double deserialized_real = stold(args.data);
                rc = sqlite3_bind_double (stmt, 9, deserialized_real); 
                break;
            }
            case ATTR_DATA_TYPE_TEXT : {
                rc = sqlite3_bind_text (stmt, 9, strdup(args.data.c_str()), -1, free);
                break;
            }
            case ATTR_DATA_TYPE_BLOB : {
                rc = sqlite3_bind_text (stmt, 9, args.data.c_str(), args.data.size()+1, SQLITE_TRANSIENT);
                break;
            }
        }
        //assert (rc == SQLITE_OK);
        ...
        rc = sqlite3_bind_int64 (stmt2, 1, attribute_id); //assert (rc == SQLITE_OK);   

        while(dim < args.num_dims) {
            rc = sqlite3_bind_double (stmt2, 2 + dim * 2, args.dims.at(dim).min); //assert (rc == SQLITE_OK);
            rc = sqlite3_bind_double (stmt2, 3 + dim * 2, args.dims.at(dim).max); //assert (rc == SQLITE_OK);
            dim++;
        }
        //if num dims < 3 explicitly bind null to prevent bleed over from prev call
        while(dim < 3) {
            rc = sqlite3_bind_null (stmt2, 2 + dim * 2); 
            rc = sqlite3_bind_null (stmt2, 3 + dim * 2); 
              dim++;
        }

        rc = sqlite3_step (stmt2); //assert (rc == SQLITE_ROW || rc == SQLITE_DONE);

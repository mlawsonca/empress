...
template <class T>
static int get_matching_timestep_count (sqlite3 *db, const md_catalog_all_timesteps_with_var_attributes_with_type_var_substr_dims_range_args &args, 
...
WaitingType OpCatalogAllTimestepsWithVarAttributesWithTypeVarSubstrDimsRangeMeta::handleMessage(bool rdma, message_t *incoming_msg) {
    ...
    //    // cout << "got here 1 \n";
            ...
            if(sql_args.query_type == COMMITTED) {
                rc = md_catalog_all_timesteps_with_var_attributes_with_type_var_substr_dims_range_stub(main_db, sql_args, min_int, max_int, entries, count);
            }
            else if(sql_args.query_type == UNCOMMITTED) {
                sqlite3 *db = get_database(sql_args.txn_id);
                rc = md_catalog_all_timesteps_with_var_attributes_with_type_var_substr_dims_range_stub(db, sql_args, min_int, max_int, entries, count);
            }
                ...
                rc = md_catalog_all_timesteps_with_var_attributes_with_type_var_substr_dims_range_stub(main_db, sql_args, min_int, max_int, entries, count);
                if(rc == RC_OK) {
                    rc = md_catalog_all_timesteps_with_var_attributes_with_type_var_substr_dims_range_stub(db, sql_args, min_int, max_int, entries, temp_count);
                    count += temp_count;
                }
            }
            else {
        ...
        case ATTR_DATA_TYPE_REAL : {
            ...
            if(sql_args.query_type == COMMITTED) {
                rc = md_catalog_all_timesteps_with_var_attributes_with_type_var_substr_dims_range_stub(main_db, sql_args, min_real, max_real, entries, count);
            }
            else if(sql_args.query_type == UNCOMMITTED) {
                sqlite3 *db = get_database(sql_args.txn_id);
                rc = md_catalog_all_timesteps_with_var_attributes_with_type_var_substr_dims_range_stub(db, sql_args, min_real, max_real, entries, count);
            }
            else if(sql_args.query_type == COMMITTED_AND_UNCOMMITTED) {
                uint32_t temp_count;
                ...
                rc = md_catalog_all_timesteps_with_var_attributes_with_type_var_substr_dims_range_stub(main_db, sql_args, min_real, max_real, entries, count);
                if(rc == RC_OK) {
                    rc = md_catalog_all_timesteps_with_var_attributes_with_type_var_substr_dims_range_stub(db, sql_args, min_real, max_real, entries, temp_count);
                    count += temp_count;
                }
            }
            else {

...
template <class T>
static int get_matching_attribute_count (sqlite3 *db, const md_catalog_all_var_attributes_with_type_var_range_args &args, 
    T min, T max, string query_str, uint32_t &count);



WaitingType OpCatalogAllVarAttributesWithTypeVarRangeMeta::handleMessage(bool rdma, message_t *incoming_msg) {
 ...
 //            rc = md_catalog_all_var_attributes_with_type_var_range_stub(db, sql_args, 
 ...
 //    }   
    ...
    switch(sql_args.data_type) {
        case ATTR_DATA_TYPE_INT : {
            ...
            get_range_values(sql_args.data, sql_args.range_type, min_int, max_int);

            if(sql_args.query_type == COMMITTED) {
                rc = md_catalog_all_var_attributes_with_type_var_range_stub(main_db, sql_args, min_int, max_int, entries, count);
            }
            else if(sql_args.query_type == UNCOMMITTED) {
                sqlite3 *db = get_database(sql_args.txn_id);
                rc = md_catalog_all_var_attributes_with_type_var_range_stub(db, sql_args, min_int, max_int, entries, count);
            }
                ...
                rc = md_catalog_all_var_attributes_with_type_var_range_stub(main_db, sql_args, min_int, max_int, entries, count);
                if(rc == RC_OK) {
                    rc = md_catalog_all_var_attributes_with_type_var_range_stub(db, sql_args, min_int, max_int, entries, temp_count);
                    count += temp_count;
                }
            }
            else {
        ...
        case ATTR_DATA_TYPE_REAL : {
            ...
            get_range_values(sql_args.data, sql_args.range_type, min_real, max_real);

            if(sql_args.query_type == COMMITTED) {
                rc = md_catalog_all_var_attributes_with_type_var_range_stub(main_db, sql_args, min_real, max_real, entries, count);
            }
            else if(sql_args.query_type == UNCOMMITTED) {
                sqlite3 *db = get_database(sql_args.txn_id);
                rc = md_catalog_all_var_attributes_with_type_var_range_stub(db, sql_args, min_real, max_real, entries, count);
            }
            else if(sql_args.query_type == COMMITTED_AND_UNCOMMITTED) {
                uint32_t temp_count;
                ...
                rc = md_catalog_all_var_attributes_with_type_var_range_stub(main_db, sql_args, min_real, max_real, entries, count);
                if(rc == RC_OK) {
                    rc = md_catalog_all_var_attributes_with_type_var_range_stub(db, sql_args, min_real, max_real, entries, temp_count);
                    count += temp_count;
                }
            }
            else {

...
template <class T1>
static int get_matching_attribute_count (sqlite3 *db, const md_catalog_all_var_attributes_with_type_var_substr_dims_range_args &args, 
    T1 min, T1 max, string query_str, uint32_t &count);



WaitingType OpCatalogAllVarAttributesWithTypeVarSubstrDimsRangeMeta::handleMessage(bool rdma, message_t *incoming_msg) {

    int rc;
    md_catalog_all_var_attributes_with_type_var_substr_dims_range_args sql_args;
    std::vector<md_catalog_var_attribute_entry> entries;
    uint32_t count;
    
    deArchiveMsgFromClient(rdma, incoming_msg, sql_args);
    add_timing_point(OP_CATALOG_ALL_VAR_ATTRIBUTES_WITH_TYPE_VAR_SUBSTR_DIMS_RANGE_DEARCHIVE_MSG_FROM_CLIENT);
    ...
    //    // cout << "got here 1 \n";
            ...
            if(sql_args.query_type == COMMITTED) {
                rc = md_catalog_all_var_attributes_with_type_var_substr_dims_range_stub(main_db, sql_args, min_int, max_int, entries, count);
            }
            else if(sql_args.query_type == UNCOMMITTED) {
                sqlite3 *db = get_database(sql_args.txn_id);
                rc = md_catalog_all_var_attributes_with_type_var_substr_dims_range_stub(db, sql_args, min_int, max_int, entries, count);
            }
                ...
                rc = md_catalog_all_var_attributes_with_type_var_substr_dims_range_stub(main_db, sql_args, min_int, max_int, entries, count);
                if(rc == RC_OK) {
                    rc = md_catalog_all_var_attributes_with_type_var_substr_dims_range_stub(db, sql_args, min_int, max_int, entries, temp_count);
                    count += temp_count;
                }
            }
            else {
        ...
        case ATTR_DATA_TYPE_REAL : {
            ...
            if(sql_args.query_type == COMMITTED) {
                rc = md_catalog_all_var_attributes_with_type_var_substr_dims_range_stub(main_db, sql_args, min_real, max_real, entries, count);
            }
            else if(sql_args.query_type == UNCOMMITTED) {
                sqlite3 *db = get_database(sql_args.txn_id);
                rc = md_catalog_all_var_attributes_with_type_var_substr_dims_range_stub(db, sql_args, min_real, max_real, entries, count);
            }
            else if(sql_args.query_type == COMMITTED_AND_UNCOMMITTED) {
                uint32_t temp_count;
                ...
                rc = md_catalog_all_var_attributes_with_type_var_substr_dims_range_stub(main_db, sql_args, min_real, max_real, entries, count);
                if(rc == RC_OK) {
                    rc = md_catalog_all_var_attributes_with_type_var_substr_dims_range_stub(db, sql_args, min_real, max_real, entries, temp_count);
                    count += temp_count;
                }
            }
            else {
    ...
    add_timing_point(OP_CATALOG_ALL_VAR_ATTRIBUTES_WITH_TYPE_VAR_SUBSTR_DIMS_RANGE_MD_CATALOG_ALL_VAR_ATTRIBUTES_WITH_TYPE_VAR_SUBSTR_DIMS_RANGE_STUB);


    std::string serial_str = serializeMsgToClient(entries, count, rc);
    add_timing_point(OP_CATALOG_ALL_VAR_ATTRIBUTES_WITH_TYPE_VAR_SUBSTR_DIMS_RANGE_SERIALIZE_MSG_FOR_CLIENT);

    createOutgoingMessage(dst, 
                          0, //Not expecting a reply
                          dst_mailbox,
                          serial_str);
    add_timing_point(OP_CATALOG_ALL_VAR_ATTRIBUTES_WITH_TYPE_VAR_SUBSTR_DIMS_RANGE_CREATE_MSG_FOR_CLIENT);

    opbox::net::SendMsg(peer, ldo_msg);
    state=State::done;
    add_timing_point(OP_CATALOG_ALL_VAR_ATTRIBUTES_WITH_TYPE_VAR_SUBSTR_DIMS_RANGE_SEND_MSG_TO_CLIENT_OP_DONE);

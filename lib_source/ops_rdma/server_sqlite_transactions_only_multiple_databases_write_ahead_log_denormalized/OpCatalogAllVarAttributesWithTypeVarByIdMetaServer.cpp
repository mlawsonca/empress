...
WaitingType OpCatalogAllVarAttributesWithTypeVarByIdMeta::handleMessage(bool rdma, message_t *incoming_msg) {
    ...
    if(sql_args.query_type == COMMITTED) {
        rc = md_catalog_all_var_attributes_with_type_var_by_id_stub(main_db, sql_args, entries, count);
    }
    else if(sql_args.query_type == UNCOMMITTED) {
        sqlite3 *db = get_database(sql_args.txn_id);
        rc = md_catalog_all_var_attributes_with_type_var_by_id_stub(db, sql_args, entries, count);
    }
        ...
        rc = md_catalog_all_var_attributes_with_type_var_by_id_stub(main_db, sql_args, entries, count);
        if(rc == RC_OK) {
            rc = md_catalog_all_var_attributes_with_type_var_by_id_stub(db, sql_args, entries, temp_count);
            count += temp_count;
        }
    }
    else {

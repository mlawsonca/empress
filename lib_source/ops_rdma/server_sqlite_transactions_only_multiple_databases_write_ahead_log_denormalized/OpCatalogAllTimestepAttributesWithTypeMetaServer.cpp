...
static int get_matching_attribute_count (sqlite3 *db, const md_catalog_all_timestep_attributes_with_type_args &args, uint32_t &count);
...
WaitingType OpCatalogAllTimestepAttributesWithTypeMeta::handleMessage(bool rdma, message_t *incoming_msg) {
    ...
    if(sql_args.query_type == COMMITTED) {
        rc = md_catalog_all_timestep_attributes_with_type_stub(main_db, sql_args, entries, count);
    }
    else if(sql_args.query_type == UNCOMMITTED) {
        sqlite3 *db = get_database(sql_args.txn_id);
        rc = md_catalog_all_timestep_attributes_with_type_stub(db, sql_args, entries, count);
    }
        ...
        rc = md_catalog_all_timestep_attributes_with_type_stub(main_db, sql_args, entries, count);
        if(rc == RC_OK) {
            rc = md_catalog_all_timestep_attributes_with_type_stub(db, sql_args, entries, temp_count);
            count += temp_count;
        }
    }
    else {
                           ...
                           uint32_t &count)
{
    ...
    const char * query_all = "select * from timestep_attribute_catalog tac "
    "where tac.run_id = ? "
    "and tac.type_id = ? ";

    const char * query = "select * from timestep_attribute_catalog tac "
    "where tac.run_id = ? "
    "and tac.timestep_id = ? "
    "and tac.type_id = ? ";

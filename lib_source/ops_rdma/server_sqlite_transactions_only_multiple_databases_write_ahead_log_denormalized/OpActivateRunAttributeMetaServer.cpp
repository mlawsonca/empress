...
WaitingType OpActivateRunAttributeMeta::handleMessage(bool rdma, message_t *incoming_msg) {
    ...
    rc = md_activate_run_attribute_stub(sql_args);
...
int md_activate_run_attribute_stub (const md_activate_args &args)
{
    ...
    // rc = sqlite3_exec (db, "release run_attribute;", callback, 0, &ErrMsg);
    // if (rc != SQLITE_OK)
    // {
    //     fprintf (stderr, "Error md_activate_run_attribute_stub: Line: %d SQL error: %s\n", __LINE__, ErrMsg);
    //     sqlite3_free (ErrMsg);
    //     sqlite3_close (db);
    // }
    ...
    rc = sqlite3_prepare_v2 (main_db, "insert into run_attribute_catalog select * from aux_db.run_attribute_catalog", -1, &stmt, &tail); //assert (rc == SQLITE_OK);
    // rc = sqlite3_prepare_v2 (main_db, "insert into run_attribute_catalog(run_id, type_id, data_type, data) select run_id, type_id, data_type, data from aux_db.run_attribute_catalog", -1, &stmt, &tail); //assert (rc == SQLITE_OK);

    rc = sqlite3_step (stmt); //assert (rc == SQLITE_ROW || rc == SQLITE_DONE);
    rc = sqlite3_finalize (stmt);  //assert (rc == SQLITE_OK); 


    rc = sqlite3_exec (main_db, "DELETE FROM aux_db.run_attribute_catalog", callback, 0, &ErrMsg);

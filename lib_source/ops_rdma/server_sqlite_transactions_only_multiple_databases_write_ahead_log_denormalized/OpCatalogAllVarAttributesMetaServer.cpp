...
WaitingType OpCatalogAllVarAttributesMeta::handleMessage(bool rdma, message_t *incoming_msg) {
    ...
    if(sql_args.query_type == COMMITTED) {
        ...
        rc = md_catalog_all_var_attributes_stub(main_db, sql_args, entries, count);
    }
    else if(sql_args.query_type == UNCOMMITTED) {
        sqlite3 *db = get_database(sql_args.txn_id);
        rc = md_catalog_all_var_attributes_stub(db, sql_args, entries, count);
    }
    else if(sql_args.query_type == COMMITTED_AND_UNCOMMITTED) {
        uint32_t temp_count;
        // cout << "OpCatalogAllVarAttributesMeta: about to query COMMITTED_AND_UNCOMMITTED" << endl;
        ...
        if(rc == RC_OK) {
            rc = md_catalog_all_var_attributes_stub(db, sql_args, entries, temp_count);
            count += temp_count;

...
static int get_matching_attribute_count (sqlite3 *db, const md_catalog_all_var_attributes_with_var_by_id_args &args, uint32_t &count);
...
WaitingType OpCatalogAllVarAttributesWithVarByIdMeta::handleMessage(bool rdma, message_t *incoming_msg) {
    ...
    if(sql_args.query_type == COMMITTED) {
        rc = md_catalog_all_var_attributes_with_var_by_id_stub(main_db, sql_args, entries, count);
    }
    else if(sql_args.query_type == UNCOMMITTED) {
        sqlite3 *db = get_database(sql_args.txn_id);
        rc = md_catalog_all_var_attributes_with_var_by_id_stub(db, sql_args, entries, count);
    }
        ...
        rc = md_catalog_all_var_attributes_with_var_by_id_stub(main_db, sql_args, entries, count);
        if(rc == RC_OK) {
            rc = md_catalog_all_var_attributes_with_var_by_id_stub(db, sql_args, entries, temp_count);
            count += temp_count;
        }
    }
    else {
                           ...
                           uint32_t &count)
{
    ...
    const char * query = "select * from var_attribute_catalog vac "
    "where (vac.var_id = ? ) "
    "and vac.run_id = ? "
    "and vac.timestep_id = ? ";

...
WaitingType OpCatalogAllTypesWithVarAttributesWithVarSubstrInTimestepMeta::handleMessage(bool rdma, message_t *incoming_msg) {
    ...
    if(sql_args.query_type == COMMITTED) {
        rc = md_catalog_all_types_with_var_attributes_with_var_substr_in_timestep_stub(main_db, sql_args, entries, count);
    }
    else if(sql_args.query_type == UNCOMMITTED) {
        sqlite3 *db = get_database(sql_args.txn_id);
        rc = md_catalog_all_types_with_var_attributes_with_var_substr_in_timestep_stub(db, sql_args, entries, count);
    }
        ...
        rc = md_catalog_all_types_with_var_attributes_with_var_substr_in_timestep_stub(main_db, sql_args, entries, count);
        if(rc == RC_OK) {
            rc = md_catalog_all_types_with_var_attributes_with_var_substr_in_timestep_stub(db, sql_args, entries, temp_count);
            count += temp_count;
        }
    }
    else {
                          ...
                          uint32_t &count)
{
    ...
    if(args.timestep_id == ALL_TIMESTEPS) {
      ...
      query_count = "select count(*) from (select distinct tc.id from type_catalog tc "
      "inner join var_attribute_catalog vac on tc.id = vac.type_id "
      "inner join var_catalog vc on vac.run_id = vc.run_id and vac.timestep_id = vc.timestep_id and vac.var_id = vc.id "
      "where tc.run_id = ? "
      "and vc.name like ? ) as internalQuery";


        rc = sqlite3_prepare_v2 (db, query_count.c_str(), -1, &stmt, &tail); //assert (rc == SQLITE_OK);

...
int sql_retrieve_var_attrs(sqlite3_stmt * stmt, std::vector<md_catalog_var_attribute_entry> &attribute_list)
{
    ...
    while (rc == SQLITE_ROW)
    {
        ...
        attribute.active = sqlite3_column_int (stmt, 5);
        attribute.txn_id = sqlite3_column_int64 (stmt, 6);
        attribute.num_dims = sqlite3_column_int (stmt, 7);
        ...
        attribute.data_type = (attr_data_type) sqlite3_column_int (stmt, 14);   
        switch(attribute.data_type) {  
            // case ATTR_DATA_TYPE_NULL : {
            //     break;
            // }        
            case ATTR_DATA_TYPE_INT : {
                attribute.data = std::to_string(sqlite3_column_int64 (stmt, 15));                                                                                
                break;
            }
            case ATTR_DATA_TYPE_REAL : {
                attribute.data = std::to_string(sqlite3_column_double (stmt, 15));                                                                                
                break;
            }
            case ATTR_DATA_TYPE_TEXT : {
                attribute.data = (char *)sqlite3_column_text (stmt, 15);
                break;
            }
            case ATTR_DATA_TYPE_BLOB : {
                int size_blob = sqlite3_column_bytes (stmt, 15);
                
                attribute.data.assign((char *)sqlite3_column_text (stmt, 15),(char *)sqlite3_column_text (stmt, 15) + size_blob);
                break;
            }
        }                         
...
int sql_retrieve_timestep_attrs(sqlite3_stmt * stmt, std::vector<md_catalog_timestep_attribute_entry> &attribute_list)
{
    ...
    while (rc == SQLITE_ROW)
    {
        ...
        attribute.data_type = (attr_data_type) sqlite3_column_int (stmt, 6);
        switch(attribute.data_type) {
            // case ATTR_DATA_TYPE_NULL : {
            //     break;
            // }        
            case ATTR_DATA_TYPE_INT : {
                attribute.data = std::to_string(sqlite3_column_int64 (stmt, 7));
                break;
            }
            case ATTR_DATA_TYPE_REAL : {
                attribute.data = std::to_string(sqlite3_column_double (stmt, 7));                    
                break;
            }
            case ATTR_DATA_TYPE_TEXT : {
                attribute.data = (char *)sqlite3_column_text (stmt, 7);
                break;
            }
            case ATTR_DATA_TYPE_BLOB : {
                int size_blob = sqlite3_column_bytes (stmt, 7);
                
                attribute.data.assign((char *)sqlite3_column_text (stmt, 7),(char *)sqlite3_column_text (stmt, 7) + size_blob);
                break;
            }
        }                        

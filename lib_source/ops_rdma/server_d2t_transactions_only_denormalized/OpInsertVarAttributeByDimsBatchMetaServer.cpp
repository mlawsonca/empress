...
int md_insert_var_attribute_by_dims_batch_stub (const vector<md_insert_var_attribute_by_dims_args> &all_args)
{
    ...
    if(all_args.size() > 0) {
        ...
        rc = sqlite3_prepare_v2 (db, "insert into var_attribute_catalog (id, run_id, timestep_id, type_id, var_id, active, txn_id, num_dims, d0_min, d0_max, d1_min, d1_max, d2_min, d2_max, data_type, data) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", -1, &stmt, &tail_index);
        if (rc != SQLITE_OK)
        {
    ...
    for(int arg_indx = 0; arg_indx < all_args.size(); arg_indx++) {
        ...
        rc = sqlite3_bind_int (stmt, 6, 0); //assert (rc == SQLITE_OK);
        rc = sqlite3_bind_int64 (stmt, 7, args.txn_id); //assert (rc == SQLITE_OK);
        ...
        while(dim < args.num_dims) {
            ...
            rc = sqlite3_bind_double (stmt, 9 + dim * 2, args.dims.at(dim).min); //assert (rc == SQLITE_OK);
            rc = sqlite3_bind_double (stmt, 10 + dim * 2, args.dims.at(dim).max); //assert (rc == SQLITE_OK);
            dim++;
        }
        //if num dims < 3 explicitly bind null to prevent bleed over from prev call
        while(dim < 3) {
            rc = sqlite3_bind_null (stmt, 9 + dim * 2); 
            rc = sqlite3_bind_null (stmt, 10 + dim * 2); 
              dim++;
        }
        rc = sqlite3_bind_int (stmt, 15, args.data_type); //assert (rc == SQLITE_OK);
        ...
        switch(args.data_type) {
            ...
            case ATTR_DATA_TYPE_INT : {
             ...
             //    //cout << "deserialized_int: " << deserialized_int << endl;
                rc = sqlite3_bind_int64 (stmt, 16, deserialized_int); 
                break;
            }
            case ATTR_DATA_TYPE_REAL : {
             ...
             //    // cout << "deserialized_real: " << deserialized_real << endl;
                rc = sqlite3_bind_double (stmt, 16, deserialized_real); 
                break;
            }
            case ATTR_DATA_TYPE_TEXT : {
             ...
             //    free(data);
                ...
                rc = sqlite3_bind_text (stmt, 16, strdup(args.data.c_str()), -1, free);
            ...
            case ATTR_DATA_TYPE_BLOB : {
                ...
                rc = sqlite3_bind_text (stmt, 16, args.data.c_str(), args.data.size()+1, SQLITE_TRANSIENT);
                // rc = sqlite3_bind_text (stmt, 15, args.data.c_str(), args.data.size(), SQLITE_STATIC);

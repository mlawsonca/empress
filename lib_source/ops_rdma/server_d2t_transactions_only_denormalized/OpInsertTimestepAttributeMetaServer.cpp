...
WaitingType OpInsertTimestepAttributeMeta::handleMessage(bool rdma, message_t *incoming_msg) {
    ...
    deArchiveMsgFromClient(rdma, incoming_msg, sql_args);        add_timing_point(OP_INSERT_TIMESTEP_ATTRIBUTE_DEARCHIVE_MSG_FROM_CLIENT);
                          ...
                          uint64_t &attribute_id)
{
    ...
    rc = sqlite3_prepare_v2 (db, "insert into timestep_attribute_catalog (id, run_id, timestep_id, type_id, active, txn_id, data_type, data) values (?, ?, ?, ?, ?, ?, ?, ?)", -1, &stmt, &tail_index);
    if (rc != SQLITE_OK)
    {
    ...
    rc = sqlite3_bind_int (stmt, 7, args.data_type); //assert (rc == SQLITE_OK);
    switch(args.data_type) {
        ...
        case ATTR_DATA_TYPE_INT : {
         ...
         //    ia >> deserialized_int;
            rc = sqlite3_bind_int64 (stmt, 8, deserialized_int); 
            break;
        }
        case ATTR_DATA_TYPE_REAL : {
         ...
         //    ia >> deserialized_real;
            rc = sqlite3_bind_double (stmt, 8, deserialized_real); 
            break;
        }
        case ATTR_DATA_TYPE_TEXT : {
            rc = sqlite3_bind_text (stmt, 8, strdup(args.data.c_str()), -1, free);
            break;
        }
        case ATTR_DATA_TYPE_BLOB : {
            // rc = sqlite3_bind_text (stmt, 7, args.data.c_str(), args.data.size()+1, free);
            rc = sqlite3_bind_text (stmt, 8, args.data.c_str(), args.data.size()+1, SQLITE_TRANSIENT);
            break;
        }
    }

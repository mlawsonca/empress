...
WaitingType OpCatalogAllRunAttributesWithTypeRangeMeta::handleMessage(bool rdma, message_t *incoming_msg) {
    ...
    switch(args.data_type) {
        case ATTR_DATA_TYPE_INT : {
            ...
            if(!trans_active) {
                rc = md_catalog_all_run_attributes_with_type_range_stub(db, args, min_int, max_int, attribute_list, count);
            }
            else {
                rc = RC_DB_BUSY;
            }
        ...
        case ATTR_DATA_TYPE_REAL : {
            ...
            sqlite3 *db = get_database(args.job_id, args.query_type, trans_active);
            if(!trans_active) {
                rc = md_catalog_all_run_attributes_with_type_range_stub(db, args, min_real, max_real, attribute_list, count);
            }
            else {
                rc = RC_DB_BUSY;
            }                        

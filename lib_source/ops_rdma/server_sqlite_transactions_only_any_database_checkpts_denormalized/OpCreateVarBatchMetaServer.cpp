...
WaitingType OpCreateVarBatchMeta::handleMessage(bool rdma, message_t *incoming_msg) {
    ...
    int rc;
    vector<md_create_var_args> args;
    // uint64_t row_id;
    ...
    sqlite3 *db = get_database_for_write(args[0].job_id, VAR_CATALOG);

    // //log("About to create var stub");
    // rc = md_create_var_batch_stub(db, args, row_id);
    rc = md_create_var_batch_stub(db, args);        

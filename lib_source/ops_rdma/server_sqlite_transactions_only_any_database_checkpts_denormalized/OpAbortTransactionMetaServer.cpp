#include <OpAbortTransactionMetaCommon.hh>
...
WaitingType OpAbortTransactionMeta::handleMessage(bool rdma, message_t *incoming_msg) {
    ...
    if (rc != SQLITE_OK)
    {
        fprintf (stderr, "Error OpAbortTransactionMeta: Line: %d SQL error: %s\n", __LINE__, ErrMsg);
        sqlite3_free (ErrMsg);
        close_database(database_to_query);
    }

    add_timing_point(OP_ABORT_TRANSACTION_MD_ABORT_TRANSACTION_STUB);
    ...
    state=State::done;
    add_timing_point(OP_ABORT_TRANSACTION_SEND_MSG_TO_CLIENT_OP_DONE);

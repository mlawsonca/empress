...
WaitingType OpCreateTypeBatchMeta::handleMessage(bool rdma, message_t *incoming_msg) {
    ...
    int rc;
    vector<md_create_type_args> args;
    ...
    // cout << "about to create type batch for job_id: " << args[0].job_id << endl;
    sqlite3 *db = get_database_for_write(args[0].job_id, TYPE_CATALOG);

    // cout << "am creating savepoint type for job_id: " << args[0].job_id << endl;

    rc = md_create_type_batch_stub(db, args, first_type_id);
...
int md_create_type_batch_stub (sqlite3 *db, const vector<md_create_type_args> &all_args, uint64_t &type_id)
{
    ...
    // cout << "md_create_type_batch_stub: about to execute savepoint type" << endl;

    //cout << "creating savepoint type" << endl;
...
//printf ("generated new global id:%d\n", *type_id);
    ...
    // cout << "done with create_type_batch_stub" << endl;

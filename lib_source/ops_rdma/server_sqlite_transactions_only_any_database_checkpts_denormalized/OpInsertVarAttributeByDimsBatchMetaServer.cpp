...
//     vector<md_insert_var_attribute_by_dims_args> args;
...
//         deArchiveMsgFromClient(false, incoming_msg, args);
//         add_timing_point(OP_INSERT_VAR_ATTRIBUTE_BY_DIMS_BATCH_DEARCHIVE_MSG_FROM_CLIENT);

//         // rc = md_insert_var_attribute_by_dims_batch_stub(db, args, attribute_ids);
//         rc = md_insert_var_attribute_by_dims_batch_stub(db, args);
...
WaitingType OpInsertVarAttributeByDimsBatchMeta::handleMessage(bool rdma, message_t *incoming_msg) {
    ...
    sqlite3 *db = get_database_for_write(args[0].job_id, VAR_ATTR_CATALOG);

...
WaitingType OpCreateTypeMeta::handleMessage(bool rdma, message_t *incoming_msg) {
    ...
    sqlite3 *db = get_database_for_write(args.job_id, TYPE_CATALOG);

    // std::cout << "About to create var stub \n";
    rc = md_create_type_stub(db, args, type_id);
                        ...
                        uint64_t &type_id)
{
    ...
    // cout << "md_create_type_stub: about to execute savepoint type" << endl;

...
WaitingType OpCatalogAllTimestepsWithVarAttributesWithTypeVarSubstrDimsMeta::handleMessage(bool rdma, message_t *incoming_msg) {
    ...
    if(!trans_active) {
        rc = md_catalog_all_timesteps_with_var_attributes_with_type_var_substr_dims_stub(db, args, entries, count);
    }
    else {
        rc = RC_DB_BUSY;
    } 
                           ...
                           uint32_t &count)
{
    ...
    // run_id 1, type_id 2, var_name_substr VAR and job_id: 5

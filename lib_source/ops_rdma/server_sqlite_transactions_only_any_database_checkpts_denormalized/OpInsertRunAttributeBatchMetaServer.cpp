...
// int md_insert_run_attribute_batch_stub (sqlite3 *db, const vector<md_insert_run_attribute_args> &args, uint64_t &attribute_id);
int md_insert_run_attribute_batch_stub (sqlite3 *db, const vector<md_insert_run_attribute_args> &args);
extern int callback (void * NotUsed, int argc, char ** argv, char ** ColName);


WaitingType OpInsertRunAttributeBatchMeta::handleMessage(bool rdma, message_t *incoming_msg) {
    
    int rc;
    vector<md_insert_run_attribute_args> args;
    // uint64_t attribute_id;

    deArchiveMsgFromClient(rdma, incoming_msg, args); 
    // sqlite3 *db = get_database(args[0].job_id);
    sqlite3 *db = get_database_for_write(args[0].job_id, RUN_ATTR_CATALOG);

    add_timing_point(OP_INSERT_RUN_ATTRIBUTE_BATCH_DEARCHIVE_MSG_FROM_CLIENT);

    // rc = md_insert_run_attribute_batch_stub(db, args, attribute_id);
    rc = md_insert_run_attribute_batch_stub(db, args);
...
// int md_insert_run_attribute_batch_stub (sqlite3 *db, const vector<md_insert_run_attribute_args> &args,
//                           uint64_t &attribute_id)
int md_insert_run_attribute_batch_stub (sqlite3 *db, const vector<md_insert_run_attribute_args> &all_args)
{
    int rc;
    int i = 0;
    ...
    // if (rc != SQLITE_OK)
    // {
    //     fprintf (stderr, "Error md_insert_run_attribute_batch_stub: Line: %d SQL error: %s\n", __LINE__, ErrMsg);
    //     sqlite3_free (ErrMsg);
    ...
    //     sqlite3_free (ErrMsg);
    //     close_database(all_args[].job_id);
    //     goto cleanup;
    // }

...
WaitingType OpCatalogAllVarAttributesWithTypeVarDimsRangeMeta::handleMessage(bool rdma, message_t *incoming_msg) {
    ...
    //    cout << "got here 1 \n";
            ...
            if(!trans_active) {
                rc = md_catalog_all_var_attributes_with_type_var_dims_range_stub(db, args, min_int, max_int, entries, count);
            }
            else {
                rc = RC_DB_BUSY;
            }           
        ...
        case ATTR_DATA_TYPE_REAL : {
            ...
            sqlite3 *db = get_database(args.job_id, args.query_type, trans_active);
            if(!trans_active) {
                rc = md_catalog_all_var_attributes_with_type_var_dims_range_stub(db, args, min_real, max_real, entries, count);
            }
            else {
                rc = RC_DB_BUSY;
            }          

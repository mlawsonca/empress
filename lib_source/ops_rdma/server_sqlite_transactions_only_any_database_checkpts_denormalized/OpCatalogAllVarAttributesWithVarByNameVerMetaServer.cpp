...
WaitingType OpCatalogAllVarAttributesWithVarByNameVerMeta::handleMessage(bool rdma, message_t *incoming_msg) {
    ...
    if(!trans_active) {
        rc = md_catalog_all_var_attributes_with_var_by_name_ver_stub(db, args, entries, count);
    }
    else {
        rc = RC_DB_BUSY;
    }
                           ...
                           uint32_t &count)
{
    ...
    // cout << "var_name: " << args.var_name << endl;
    // cout << "var_version: " << args.var_version << endl;

...
WaitingType OpActivateRunAttributeMeta::handleMessage(bool rdma, message_t *incoming_msg) {
    ...
    int rc = md_activate_run_attribute_stub(args);
...
int md_activate_run_attribute_stub (const md_activate_args &args)
{
    ...
    sqlite3 *db = get_database_and_status(args.job_id, RUN_ATTR_CATALOG, savept_active);

    if(savept_active) {
        //cout << "about to release run_attribute" << endl;
